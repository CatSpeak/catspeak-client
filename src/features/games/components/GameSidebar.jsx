...
const GameSidebar = () => {
  const { scores, correctPlayers, gameState, currentUserId, playerNames, leftPlayers, gameType, pictureIt } = useGame()
  const { t } = useLanguage()
  const { user } = useAuth()
  ...
  // Use pictureIt.leaderboard if available and playing Picture IT
  let players = []
  if (isPictureIt && pictureIt?.leaderboard) {
    players = pictureIt.leaderboard.map(p => ({
      id: p.id.toString(),
      name: p.name,
      isYou: p.id.toString() === currentUserId?.toString(),
      score: p.totalScore,
      isCorrect: false, // Not used in Picture IT
      hasLeft: leftPlayers?.has(p.id.toString()) || false,
    }))
  } else {
    players = Object.keys(scores).map(id => {
  ...
  return (
    <div className="h-full w-full bg-white rounded-2xl md:rounded-3xl overflow-hidden shadow-sm border border-gray-100 p-4 md:p-6 flex flex-col">
