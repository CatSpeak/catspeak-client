...
import { X, Users, LogOut, UserPlus, Trash2 } from "lucide-react"
...
const ChatUserPanel = ({
  ...
  const containerClasses = isDrawer
    ? "w-[360px] h-full flex flex-col border-l border-t-0 border-b-0 border-r-0 shrink-0 overflow-hidden"
    : "w-[340px] h-full flex flex-col justify-start shrink-0 overflow-hidden"
  ...
  return (
    <Container padding="p-0" className={containerClasses}>
      ...
      <div className="flex items-center justify-between px-4 h-[72px] border-b border-[#E5E5E5] shrink-0">
        <h3 className="font-semibold">{isGroup ? "Group Info" : "Profile"}</h3>
        <IconButton
          onClick={onClose}
          variant="ghost"
          size="sm"
