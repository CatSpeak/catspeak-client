...
const EventBlockDetail = ({ event, open, onClose }) => {
  ...
  const EVENT_STYLES = {
    "teaching-schedule": {
      background: "#f1fff8",
      label: t.calendar?.teachingSchedule || "Lịch dạy",
      color: "#34ce56",
    },
    "student-schedule": {
      background: "#f0f5ff",
      label: t.calendar?.studentSchedule || "Lịch học",
      color: "#0e6eec",
    },
    "my-event": {
      background: "#ffeef0",
      label: t.calendar?.myEvent || "Sự kiện của tôi",
      color: "#f83b4f",
    },
    "registered-event": {
      background: "#fffceb",
      label: t.calendar?.registeredEvent || "Đã đăng ký",
      color: "#e2b60a",
    },
    other: {
      background: "#ffffff",
      label: t.calendar?.other || "Khác",
      color: "#888888",
    },
  };
  ...
  const handleNavigate = (event) => {
    switch (event.eventType) {
      ...
      case "my-event":
        return navigate(`/workspace/my-calendar`, {
  ...
  const renderImage = (event) => {
    if (["teaching-schedule", "student-schedule"].includes(event.eventType))
      return null;
      ...
      case "my-event": {
        ...
        if (isPast) {
          ...
          statusLabel = isUpcoming
            ? t.calendar?.upcoming || "Sắp diễn ra"
            : t.calendar?.ongoing || "Đang diễn ra";
        } else {
      ...
      case "student-schedule":
        return (
          <div className="p-4 border-t border-border flex items-center justify-center gap-3 bg-white shrink-0">
            ...
            <PillButton onClick={() => handleNavigate(event)}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-[#1A1A1A] font-medium">
                {t.courses?.student?.classStatuses?.[event.status] ||
                  event.status}
              </p>
              <p className="text-sm text-gray-500">
                {t.calendar?.statusLabel || "Trạng thái"}
              </p>
            </div>
          </div>
        )}
