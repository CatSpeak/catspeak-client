...
const CalendarWeekView = ({ currentDate, selectedDate, events }) => {
  ...
  const DAY_LABELS = ["MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN"].map(
    (day) => formatScheduleDays([day]),
  );
  ...
  const isCurrentWeek = weekDates.some(
    (d) => d.dateStr === now.format("YYYY-MM-DD"),
  );
      ...
      <div className="flex ml-16 border-b border-border pb-4">
        {weekDates.map((dateObj, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`w-12 h-12 flex items-center justify-center rounded-full text-sm font-semibold tracking-wider transition-all 
        ...
        <div className="flex relative pt-3">
          ...
          <div className="w-16 flex flex-col shrink-0">
            {HOURS.map((hour) => (
              <div
                key={hour}
                className="h-[60px] text-xs text-gray-400 font-medium relative -top-2"
              >
                {hour}:00
              </div>
            ))}
          ...
          <div className="flex-1 relative">
            {HOURS.map((_, i) => (
              <div
                key={i}
                className="h-[60px] border-t border-[#F5F5F5] w-full"
              />
            ))}
                  ...
                  {(() => {
                    const parsedEvents = events
                      .map((ev) => {
                        ...
                        if (
                          !evStart.isBefore(colEnd) ||
                          !evEnd.isAfter(colStart)
                        )
                          return null;

                        // Calculate the start and end time visually restricted within this day column
                        const renderStart = evStart.isBefore(colStart)
                          ? colStart
                          : evStart;
                        const renderEnd = evEnd.isAfter(colEnd)
                          ? colEnd
                          : evEnd;
                    ...
                    parsedEvents.sort(
                      (a, b) =>
                        a.renderStart.valueOf() - b.renderStart.valueOf(),
                    );
                    ...
                    parsedEvents.forEach((pEv) => {
                      if (
                        clusterEnd === null ||
                        !pEv.renderStart.isBefore(clusterEnd)
                      ) {
          ...
          {showCurrentTime && (
            <div
              className="absolute left-0 right-0 border-t border-red-500 flex items-center"
