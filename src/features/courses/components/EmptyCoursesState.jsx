...
const EmptyCoursesState = ({
  ...
  return (
    <div
      className={`relative overflow-hidden max-w-xl w-full mx-auto my-4 p-8 sm:p-10 rounded-2xl border border-slate-200/90 bg-white shadow-xs flex flex-col items-center justify-center text-center transition-all ${className}`}
    >
      ...
      {/* Icon Badge */}
      <div className="relative z-10 w-14 h-14 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center mb-4 text-[#b20a1c] shadow-2xs">
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-center gap-3 mb-6">
          ...
          {isFiltered && onResetFilter && (
            <button
              ...
              className="h-9 px-4 rounded-lg border border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs flex items-center gap-2 transition-all shadow-2xs cursor-pointer active:scale-98"
            >
        ...
        <div className="relative z-10 pt-5 border-t border-slate-100 w-full flex flex-wrap justify-center items-center gap-2">
          {tips.map((tip, idx) => {
            ...
            return (
              <div
                key={idx}
                className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-slate-50 border border-slate-200/80 text-[11px] font-medium text-slate-600"
              >
