...
const CourseActionMenu = ({ item, labels, onEdit, onDelete }) => {
  ...
  return (
    <div ref={menuRef} className="relative" onClick={(event) => event.stopPropagation()}>
      ...
      {isOpen && (
        <div role="menu" className="absolute right-0 mt-1 w-36 bg-white border border-border rounded-2xl shadow-lg py-1 z-30 text-left">
          <button
            type="button"
            role="menuitem"
            onClick={(event) => {
...
const CourseManagementCard = ({
  ...
  return (
    <div
      onClick={() => onOpen(item)}
      className={`bg-white rounded-3xl border border-border hover:border-border overflow-hidden shadow-xs hover:shadow-md transition-all duration-300 cursor-pointer flex justify-between ${isGrid ? "flex-col min-h-[460px]" : "flex-row items-center p-4 gap-6"}`}
    >
      <CourseThumbnail
        item={item}
        title={item.title}
      ...
      <div className="p-6 flex flex-col flex-1 justify-between gap-4">
            ...
            <>
              ...
              <div className="mt-1.5 flex items-center gap-2">
                {item.courseId && item.courseTitle ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-slate-600 bg-slate-100 px-2.5 py-0.5 rounded-md border border-border/60">
        ...
        {isCourse && (
          <div className="pt-4 border-t border-border flex flex-col gap-2 text-xs font-bold">
