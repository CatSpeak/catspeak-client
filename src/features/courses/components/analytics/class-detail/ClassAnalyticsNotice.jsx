...
const ClassAnalyticsNotice = ({ teacherName = "Minh Hoàng" }) => {
  const { t } = useLanguage()
  const c = t.courses || {}
  const cd = c.analytics?.classDetail || {}

  const disclaimerText = cd.teacherDisclaimer
    ? cd.teacherDisclaimer.replace("{{name}}", teacherName)
    : `GV ${teacherName} không được tính vào STB — chỉ thời gian phát biểu của học viên được thống kê.`
