...
import { useGetUserProfileQuery, useUpdateMeetingAvatarMutation } from "@/store/api/userApi"
...
const AvatarUrlPicker = ({ className = "p-4" }) => {
  ...
  const currentAvatarUrl = profileData?.data?.meetingAvatarUrl || profileData?.data?.avatarImageUrl || ""
  ...
  const handleSave = async () => {
    try {
      ...
      toast.success(t?.rooms?.avatarPicker?.success || "Avatar updated successfully")
    } catch (err) {
    ...
    <div className={`flex flex-col h-full w-full ${className}`}>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex justify-center">
          <Avatar size={120} src={inputUrl || currentAvatarUrl} className="border-2 border-white shadow-md" />
        </div>
