...
const RoomsBannerContent = ({ sessionProps }) => {
  ...
  const renderTitle = () => {
    ...
    if (firstSpaceIndex !== -1) {
      ...
      return (
        <span className="inline-flex flex-wrap gap-2 md:gap-3">
          <span>{firstPart}</span>
  ...
  return (
    <div className="col-span-12 md:col-span-6 flex flex-col justify-center px-4 sm:px-8 py-4 md:py-0 md:pr-4">
      {/* Doodle logo */}
      <img
        src={doodles}
      ...
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-gray-600 text-base md:text-lg mt-3 md:mt-4 leading-relaxed max-w-full">
            {t.rooms?.welcome?.description?.part1 ||
              "Halloween is nominally a "}
            <strong className="text-cath-red-500 font-semibold">
              {t.rooms?.welcome?.description?.highlight1 || "Christian holiday"}
            </strong>
            {t.rooms?.welcome?.description?.part2 ||
              " honoring the souls of saints and other souls who have been blessed. It descends from an "}
            <strong className="text-cath-red-500 font-semibold">
              {t.rooms?.welcome?.description?.highlight2 ||
                "ancient Celtic festival"}
            </strong>
            {t.rooms?.welcome?.description?.part3 ||
          ...
          <p className="text-gray-500 text-xl md:text-2xl mt-2 font-bold italic">
            {t.rooms?.welcome?.trickOrTreat || "Trick or Treat"}
          </p>
        </div>
