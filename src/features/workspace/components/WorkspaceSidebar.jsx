...
import { Video, Film, BookOpen } from "lucide-react"
...
const WorkspaceSidebar = ({ onClose, variant = "vertical" }) => {
  ...
  const getLinkClasses = ({ isActive }) => {
    if (isHorizontal) {
      return `relative flex items-center justify-center gap-2 px-4 h-10 whitespace-nowrap transition-colors flex-1 min-w-fit ${isActive
    ...
    return `relative flex w-full items-center gap-3 px-4 h-10 rounded-r-lg transition-colors mb-1 overflow-hidden ${isActive
  ...
  const menuItems = [
    {
      label: t?.courses?.title || "My Courses",
