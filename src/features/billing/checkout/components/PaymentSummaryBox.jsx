...
const getLocalizedInterval = (interval, lang) => {
  const isMonthly = String(interval || "")
    .toLowerCase()
    .includes("month");
  if (lang === "vi") {
...
const PaymentSummaryBox = ({ t, language, plan, isProcessing, onSubmit }) => {
  ...
  return (
    <div className="bg-white border border-border rounded-3xl p-6 shadow-sm sticky top-24 space-y-6">
      ...
      {/* Package Banner */}
      <div
        className="rounded-r-xl rounded-l-xs p-4 flex items-center justify-between border-l-[5px] shadow-2xs"
        style={{
      ...
      >
        <span className="font-bold text-base tracking-wide">{plan.name}</span>
        <span
          className="text-xs font-bold px-2.5 py-1 rounded-md border"
          style={{
            backgroundColor: hexToRgbA(cardColor, 0.12),
            borderColor: hexToRgbA(cardColor, 0.25),
            color: cardColor,
          }}
        >
      ...
      <div className="space-y-3 pt-2">
        ...
        <div className="flex justify-between items-baseline">
          <span className="text-sm font-bold text-gray-900">
            {t.billing?.checkout?.total || "Tổng thanh toán"}
          </span>
          <span className="text-xl font-extrabold text-[#BE0015]">
            {formattedTotal}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-start gap-2.5 pt-4">
        ...
        <label
          ...
          className="text-xs text-gray-500 leading-normal select-none cursor-pointer"
        >
          {language === "vi" ? (
            <>
              Tôi đồng ý với{" "}
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[#BE0015] font-semibold hover:underline"
              >
      ...
      <div>
        <button
          ...
          className={`w-full py-3.5 px-6 rounded-2xl font-bold text-sm transition-all duration-150 flex items-center justify-center gap-2 active:scale-[0.99] ${
            !agreed || isProcessing
