...
const ReelCaption = React.memo(({ reel, isMobile = false }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`leading-[1.6] ${isMobile ? "text-white text-[13px] drop-shadow-md" : "text-headingColor text-[14px]"}`}
      >
        {reel.title && (
          <h3
            className={`font-bold mb-2 ${isMobile ? "text-[14px]" : "text-[15px] text-headingColor"}`}
          >
            {reel.title}
          </h3>
        )}
        <p
          className={`relative whitespace-pre-wrap break-words mb-3 ${isMobile ? "opacity-95" : "text-gray-800"}`}
        >
          {displayDescription}
          {shouldTruncate && (
            <button
      ...
      {reel.tags && reel.tags.length > 0 && (
        <div
          className={`flex flex-wrap mb-4 ${isMobile ? "gap-1.5" : "gap-1.5"}`}
        >
          {reel.tags.map((tag) => (
            <span
              key={tag}
              className={`font-medium hover:underline cursor-pointer transition-colors duration-200 ${isMobile ? "text-white text-[12px] drop-shadow-md" : "text-cath-red-700 text-[13px]"}`}
            >
