...
export const detectDescriptionTrigger = (
  value,
...
export const CreateReelProvider = ({ children, open, onClose, challenge }) => {
  ...
  const [activeDescriptionTrigger, setActiveDescriptionTrigger] =
    useState(null);
  ...
  const { tasks, startTask } = useGlobalTask();
  ...
  const handleVideoSelect = (file) => {
    ...
    if (!file.type.startsWith("video/")) {
      setValidationErrors((prev) => ({
        ...prev,
        video:
          t.catSpeak?.reels?.videoFormatError ||
    ...
    if (file.size > maxSize) {
      setValidationErrors((prev) => ({
        ...prev,
        video:
          t.catSpeak?.reels?.videoSizeError ||
          "Video file size exceeds 50MB limit.",
      }));
      return;
    }
  ...
  const handleCoverSelect = (file) => {
    ...
    if (!file.type.startsWith("image/")) {
      setValidationErrors((prev) => ({
        ...prev,
        cover:
          t.catSpeak?.reels?.coverFormatError ||
    ...
    if (file.size > maxSize) {
      setValidationErrors((prev) => ({
        ...prev,
        cover:
          t.catSpeak?.reels?.coverSizeError ||
  ...
  const handleSubmit = async (e) => {
    ...
    if (!videoFile) {
      errors.video =
        t?.catSpeak?.reels?.videoRequiredError || "A video file is required.";
    }
    if (!title.trim()) {
      errors.title =
        t?.catSpeak?.reels?.titleRequiredError || "A title is required.";
    }
    ...
    try {
      ...
      try {
        ...
        const fallbackMsg =
          t?.catSpeak?.reels?.monthlyQuotaExceeded ||
          "Bạn đã đạt giới hạn tải lên tối đa 5 Reels trong tháng này. Vui lòng nâng cấp lên gói Pro để đăng thêm Reels!";
        const errMsg =
          checkErr?.data?.message || checkErr?.message || fallbackMsg;
      ...
      const createdTaskId = startTask({
        ...
        onError: (err) => {
          const errMsg =
            err?.message ||
            t?.catSpeak?.reels?.uploadFailed ||
            "Failed to upload Reel. Please try again.";
          toast.error(errMsg);
        },
      });
      setCurrentUploadId(createdTaskId);
  ...
  const value = {
    ...
    setDescription,
    activeDescriptionTrigger,
    setActiveDescriptionTrigger,
    debouncedDescriptionTrigger,
    setDebouncedDescriptionTrigger,
    activeSuggestionIndex,
    setActiveSuggestionIndex,
    privacy,
    setPrivacy,
    ...
    handleDescriptionCursorUpdate,
    handleDescriptionScroll,
    applyDescriptionSuggestion,
    handleDescriptionKeyDown,
    handleDescriptionProtectedClipboard,
    handleSubmit,
