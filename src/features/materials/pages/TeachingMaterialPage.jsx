...
import {
  downloadFolderAsZip,
  downloadMultipleItemsAsZip,
} from "@/features/materials/utils/zipDownloader";
...
const TeachingMaterialPage = () => {
  ...
  const { data: impactData, isFetching: isImpactFetching } =
    useGetDeletionImpactQuery(impactFolderId, { skip: !impactFolderId });
  ...
  const actualSortBy = ["name", "size"].includes(sortField)
    ? sortField
    : "date";
  const actualSortOrder = sortField === "oldest" ? "asc" : sortDir || "desc";

  const { data: regularMaterialsData, isLoading: isRegularLoading } =
    useGetPersonalMaterialsQuery(
      {
        folderId,
        keyword: searchQuery,
  ...
  const { data: bookmarkedMaterialsData, isLoading: isBookmarkLoading } =
    useGetBookmarkedMaterialsQuery(
      {
        keyword: searchQuery,
        sortBy: actualSortBy,
  ...
  const materialsData =
    filterMode === "bookmark" && !folderId
      ? bookmarkedMaterialsData
      : regularMaterialsData;
  const isLoading =
    filterMode === "bookmark" && !folderId
      ? isBookmarkLoading
      : isRegularLoading;

  const { data: folderDetailRes } = useGetPersonalMaterialByIdQuery(folderId, {
    skip: !folderId,
  });
  ...
  const getFolderPath = (nodes, targetId, currentPath = []) => {
    for (const node of nodes) {
      const isMatch =
        String(node.folderId) === String(targetId) ||
        String(node.id) === String(targetId);
      const pathWithCurrent = [
        ...currentPath,
        { folderId: node.folderId || node.id, folderName: node.folderName },
      ];
  ...
  const handleBookmark = async (item, type) => {
    try {
      if (type === "folder") {
        ...
        toast.success(
          item.isBookmarked
            ? t.materials.unbookmarkFolderSuccess
            : t.materials.bookmarkFolderSuccess,
        );
      } else {
        await bookmarkMaterial(item.id).unwrap();
        toast.success(
          item.isBookmarked
            ? t.materials.unbookmarkFileSuccess
            : t.materials.bookmarkFileSuccess,
        );
      }
    } catch (error) {
      console.error(error);
  ...
  const handleToggleSelect = (item, type) => {
    setSelectedItems((prev) => {
      ...
      const exists = prev.find(
        (i) => (i.id || i.folderId) === id && i._type === type,
      );
      if (exists) {
        return prev.filter(
          (i) => !((i.id || i.folderId) === id && i._type === type),
  ...
  const folders = useMemo(() => {
    return Array.isArray(responseData.folders)
      ? responseData.folders.map((f) => ({ ...f, id: f.folderId || f.id }))
      : [];
  }, [responseData.folders]);

  const files = useMemo(() => {
    return Array.isArray(responseData.materials)
  ...
  return (
    <div
      ...
      onContextMenu={(e) => {
        if (
          e.target.closest(
            'button, input, a, .group, [role="button"], .cursor-pointer',
          )
        ) {
          return;
        }
    ...
    >
        ...
        items={[
          ...
          ...(folderPath
            ? folderPath.map((folder) => ({
                ...
                onClick: () =>
                  navigate(`/workspace/materials/${folder.folderId}`),
              }))
            : folderId
              ? [
                  {
      ...
      {/* Header actions */}
      <div className="mb-4 flex w-full flex-row gap-2 md:hidden">
        <PillButton
          variant="outline"
          roundedClass="rounded-xl"
          borderColor="#6E0009"
          textColor="#6E0009"
          startIcon={<FolderPlus className="h-4 w-4 shrink-0" />}
        ...
        <PillButton
          roundedClass="rounded-xl"
          bgColor="#6E0009"
          startIcon={<Upload className="h-4 w-4 shrink-0" />}
          className="!h-10 min-w-0 flex-1 px-2 text-sm"
      ...
      {/* Search and Filters */}
      <div className="mb-4 flex w-full flex-col gap-3 md:flex-row md:items-center md:justify-between md:gap-4">
        <div className="flex w-full min-w-0 flex-col gap-3 sm:flex-row sm:items-center md:flex-1 md:gap-4 md:px-0 px-2">
          <TextInput
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder={t.materials.searchPlaceholder}
            containerClassName="w-full min-w-0 md:max-w-[448px]"
            className="!h-10 !w-full !rounded-xl border-[#E3BEBA] py-[22px]"
            icon={Search}
          />
          ...
          <PillButton
            ...
            className="!h-10 hidden shrink-0 md:inline-flex"
            startIcon={<ListFilter className="h-4 w-4 shrink-0" />}
        ...
        <div className="flex w-full min-w-0 items-center gap-2 md:w-auto md:gap-3 md:pr-0 pr-2">
          <PillButton
            variant={filterMode ? "solid" : "outline"}
            roundedClass="rounded-xl"
            borderColor={filterMode ? "transparent" : "#E3BEBA"}
            bgColor={filterMode ? "#6E0009" : undefined}
            textColor={filterMode ? "white" : "#5B403E"}
            className="!h-10 min-w-0 flex-1 px-2 text-sm md:hidden"
            startIcon={<ListFilter className="h-4 w-4 shrink-0" />}
            onClick={() => setIsFilterModalOpen(true)}
          >
            <span className="truncate  max-[430px]:hidden ">
              {t.materials.filter} {filterMode ? `(1)` : ""}
            </span>
          </PillButton>
          <div className="flex h-10 shrink-0 items-center rounded-xl border border-[#E3BEBA] bg-[#F3F3F3] p-1">
            <IconButton
              variant="iconOnly"
              size="sm"
              aria-label={t.materials.gridView || "Grid view"}
              className={`!h-[30px] !rounded-[8px] px-3 transition-colors hover:bg-[#E3BEBA] ${viewLayout === "grid" ? "bg-[#F9F9F9]" : ""}`}
              onClick={() => setViewLayout("grid")}
            >
              <LayoutGrid
                className={`h-4 w-4 ${viewLayout === "grid" ? "text-[#6E0009]" : "text-[#5B403E]"}`}
              />
            </IconButton>
            ...
            <IconButton
              variant="iconOnly"
              size="sm"
              aria-label={t.materials.listView || "List view"}
              className={`!h-[30px] !rounded-[8px] px-3 transition-colors hover:bg-[#E3BEBA] ${viewLayout === "list" ? "bg-[#F9F9F9]" : ""}`}
              onClick={() => setViewLayout("list")}
            >
              <TableProperties
                className={`h-4 w-4 ${viewLayout === "list" ? "text-[#6E0009]" : "text-[#5B403E]"}`}
              />
            </IconButton>
          </div>
          ...
          <Dropdown
            ...
            trigger={(isOpen, selectedOption, toggle) => (
              <PillButton
                ...
                endIcon={
                  <ChevronDown
                    className={`h-4 w-4 shrink-0 text-[#5B403E] transition-transform ${isOpen ? "rotate-180" : ""}`}
                  />
                }
              >
      ...
      {isLoading ? (
          ...
          {searchQuery && (
            <div className="mb-8">
              <h2 className="text-2xl font-semibold text-[#1A1C1C]">
                {t.materials.searchResults}
              </h2>
              <p className="text-base text-[#5B403E]">
                {t.materials.zeroResults}{" "}
                <span className="font-bold text-[#6E0009]">
                  "{searchQuery}"
                </span>
              </p>
            </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-4 opacity-70">
          ...
          <p className="text-lg font-medium text-[#5B403E]">
        ...
        <>
          {searchQuery && materials.length > 0 && (
            <div className="mb-8 mt-4">
              ...
              <p className="text-base text-[#5B403E]">
                {t.materials.foundResults.replace(
                  "{{count}}",
                  materials.length,
                )}{" "}
                <span className="font-bold text-[#6E0009]">
              ...
              >
                {folders.map((folder) => (
                  <FolderItem
                    ...
                    onClick={() =>
                      navigate(
                        `/workspace/materials/${folder.id || folder.folderId}`,
                      )
                    }
                    onDownload={() =>
                      downloadFolderAsZip(folder, false, null, t)
                    }
                    onMove={() => {
          ...
          {files.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-semibold text-[#1A1C1C]">
                {searchQuery ? t.materials.files : t.materials.recentFiles}
              </h2>
              <div
                className={
                  viewLayout === "grid"
                    ? "grid grid-cols-2 lg:grid-cols-4 gap-4 xl:grid-cols-6 2xl:grid-cols-7"
                    : "flex flex-col gap-3"
                }
              >
                {files.map((file) => (
                  <FileItem
                    ...
                    size={formatSize(
                      file.fileSize || file.size || file.sizeBytes,
                    )}
                    ...
                    onDownload={async () => {
                      if (file.fileUrl) {
                        ...
                        try {
                          ...
                          console.error(
                            "Download failed, falling back to new tab:",
                            error,
                          );
          ...
          deletingItem.type === "folder"
            ? {
                ...
                affectedDetail: impactData
                  ? {
                      folders:
                        (impactData?.data || impactData)?.subFolderCount ?? 0,
                      materials:
                        (impactData?.data || impactData)?.materialCount ?? 0,
                    }
                  : null,
              }
            : deletingItem
      ...
      <FileDetailModal
        ...
        onDelete={() => {
          ...
          setDeletingItem({
            id: selectedItem.id,
            name: selectedItem.fileName || selectedItem.name,
            count: 0,
            type: "file",
          });
          setIsDeleteFolderOpen(true);
          ...
          selectedItems.length > 0 && !selectedItem
            ...
            : selectedItem
              ? [
                  {
                    ...selectedItem,
                    _type: selectedItem.fileName ? "file" : "folder",
                  },
                ]
              : []
        }
      ...
      <ContextMenu
        ...
        options={[
          {
            label: t.materials.createFolder,
            icon: <FolderPlus className="w-4 h-4" />,
            onClick: () => setIsCreateFolderOpen(true),
          },
          {
            label: t.materials.uploadMaterial,
      ...
      <BulkActionBar
          ...
          if (selectedItems.length === 1) {
            setDeletingItem({
              ...
              name:
                ...
                selectedItems[0].folderName ||
                selectedItems[0].name,
              count:
                selectedItems[0]._type === "folder"
                  ? (selectedItems[0].subFolderCount || 0) +
                    (selectedItems[0].materialCount || 0)
                  : 0,
            ...
            setDeletingItem({
              id: "bulk",
              name: t.materials.selectedItemsCount.replace(
                "{{count}}",
        ...
        onDownload={() => {
          const filesToDownload = selectedItems.filter(
            (item) => item._type === "file" && item.fileUrl,
          );
          const foldersToDownload = selectedItems.filter(
            (item) => item._type === "folder",
          );
