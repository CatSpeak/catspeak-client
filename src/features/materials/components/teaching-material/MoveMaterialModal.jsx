...
const MoveMaterialModal = ({
  ...
  const { data: treeData, isLoading: isFetchingTree } = useGetFolderTreeQuery(
    undefined,
    { skip: !open },
  )
  const [moveMaterial, { isLoading: isMovingSingle }] =
    useMoveMaterialMutation()
  const [moveFolder, { isLoading: isMovingFolder }] = useMoveFolderMutation()
  const [moveMaterialsBulk, { isLoading: isMovingBulk }] =
    useMoveMaterialsBulkMutation()
  ...
  const getFolderPath = useCallback(function getFolderPath(
    nodes,
    targetId,
    currentPath = [],
  ) {
  ...
  const currentLocation = useMemo(() => {
    ...
    if (ownPath) {
      if (ownPath.length > 1) {
        ...
        return (
          t.materials.rootFolder +
          " / " +
          parentPath.map((p) => p.folderName).join(" / ")
        )
      }
  ...
  const folders = useMemo(() => {
    ...
    const filterTree = (nodes) => {
      return nodes.reduce((acc, node) => {
        if (
          selectedFolderIds.includes(String(node.folderId)) ||
          selectedFolderIds.includes(String(node.id))
        )
          return acc
  ...
  const currentName =
    items.length === 1
      ...
      : (() => {
          const foldersCount = items.filter(
            (i) => i._type === "folder" || (i.folderId && !i.fileName),
          ).length
          const filesCount = items.filter(
            (i) => i._type === "file" || i.fileName,
          ).length
          const parts = []
          if (foldersCount > 0)
            parts.push(
              t.materials.folderCountUnit.replace("{{count}}", foldersCount),
            )
          if (filesCount > 0)
            parts.push(
              t.materials.fileCountUnit.replace("{{count}}", filesCount),
            )
  ...
  const footer = (
    <div className="flex items-center justify-end">
      <PillButton
        ...
        loading={isMoving}
        disabled={
          String(selectedFolder ? selectedFolder.folderId : null) ===
    ...
    >
      <div className="space-y-4">
        <div>
          ...
          <p className="text-secondary text-sm">
            {t.materials.currentLocationLabel}{" "}
            <span className="font-medium text-black">{currentLocation}</span>
          </p>
        </div>

        <div className="border border-border rounded-xl overflow-y-auto min-h-[250px] max-h-[400px] flex flex-col">
          {isFetchingTree ? (
            <div className="flex flex-col items-center justify-center flex-1 text-sm text-secondary gap-2 opacity-70">
              <LoadingSpinner />
            ...
            <div className="flex flex-col gap-1 p-4">
              <FolderNode
                ...
                onSelect={() =>
                  setSelectedFolder({
                    folderId: null,
                    folderName: t.materials.rootFolder,
                  })
                }
