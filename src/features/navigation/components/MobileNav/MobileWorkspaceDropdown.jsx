...
const MobileWorkspaceDropdown = ({ navKey, onClose }) => {
  ...
  const subItems = [
    ...
    { key: "reels", label: t?.catSpeak?.sidebar?.reels || "Reels", icon: Film, path: "/workspace/reels" },
  ]
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Collapse Container */}
      <div
        className={`overflow-hidden transition-all duration-300 ease-in-out ${open ? "max-h-[120px] opacity-100" : "max-h-0 opacity-0"
          }`}
      >
            ...
            return (
              ...
              >
                <div className="flex-shrink-0 min-w-[32px]">
                  <Icon size={18} className={isItemActive ? "text-cath-red-700" : ""} />
                </div>
