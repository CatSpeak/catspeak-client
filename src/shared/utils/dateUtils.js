...
/**
 * Format natural localized relative time (e.g. "vài giây trước", "5 phút trước", "2 hours ago", "1个月前")
 */
export const formatTimeAgo = (timestamp, language = "en") => {
  const date = ensureDate(timestamp)
  if (!date) return ""
  const l = String(language).toLowerCase()
  const dayjsLocale = l.startsWith("vi")
    ? "vi"
    : l.startsWith("zh")
      ? "zh-cn"
      : "en"
  const str = dayjs(date).locale(dayjsLocale).fromNow()
  if (!str) return ""
