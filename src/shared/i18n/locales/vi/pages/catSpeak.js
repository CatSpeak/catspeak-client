export default {
  anonymous: "Ẩn danh",
  connect: "Kết nối",
  pass: "Bỏ qua",
  report: "Báo cáo",
  interested: "Đã gửi quan tâm! Kiểm tra tin nhắn của bạn.",
  hidden: "Đã ẩn câu chuyện.",
  interactionFailed: "Tương tác thất bại.",
  passConfirmationTitle: "Xác nhận bỏ qua",
  inappropriateLanguage:
    "Người này có sử dụng ngôn ngữ không phù hợp trong môi trường học thuật không?",
  confirm: "Xác nhận",
  cancel: "Hủy",
  alreadyInteracted: "Bạn đã tương tác với câu chuyện này rồi.",
  discover: {
    title: "Khám phá thế giới",
    description:
      "Sắp ra mắt: Khám phá các nền văn hóa, địa điểm và con người từ khắp nơi trên thế giới.",
  },
  reportSuccess: "Báo cáo thành công",
  reportFailed: "Báo cáo thất bại",
  storyNotFound: "Không tìm thấy câu chuyện",
  cannotReportOwn: "Không thể báo cáo câu chuyện của chính mình",
  reportedWarning: "Bị báo cáo",
  reportedTooltip: "Thư này đã bị người dùng báo cáo vi phạm",
  myReportedTooltip: "Thư của bạn đã bị người dùng báo cáo",
  reels: {
    title: "Reels",
    description: "Xem video từ cộng đồng của chúng tôi.",
    loginRequired: "Vui lòng đăng nhập để đăng tải",
    videoFormatError: "Định dạng video không được hỗ trợ.",
    videoSizeError: "Kích thước video vượt quá giới hạn 50MB.",
    coverFormatError: "Định dạng ảnh bìa không được hỗ trợ.",
    coverSizeError: "Kích thước ảnh bìa vượt quá giới hạn 5MB.",
    videoRequiredError: "Vui lòng chọn một tệp video.",
    titleRequiredError: "Vui lòng nhập tiêu đề.",
    uploadFailed: "Đăng tải Reel thất bại. Vui lòng thử lại.",
    uploadSuccess: "Đăng tải Reel thành công",
    uploadModerating: "Đã tải Reel lên. Video đang được kiểm duyệt và sẽ hiển thị công khai khi hoàn tất.",
    monthlyQuotaExceeded: "Bạn đã đạt giới hạn tải lên tối đa 5 Reels trong tháng này. Vui lòng nâng cấp lên gói Pro để đăng thêm Reels!",
    uploadFailedTitle: "Tải lên thất bại",
    uploadFailedDesc: "Đã xảy ra lỗi. Vui lòng kiểm tra định dạng tệp và mạng của bạn.",
    foryou: "Dành cho bạn",
    challenges: "Thử thách",
    active: "Đang diễn ra",
    past: "Đã kết thúc",
    featuredChallenges: "Thử thách nổi bật",
    featuredChallengesDescActive: "Tham gia ngay các thử thách đang diễn ra",
    featuredChallengesDescPast: "Tham gia ngay các thử thách đã kết thúc",
    joinNow: "Tham gia ngay",
    daysLeft: "Còn {{days}} ngày",
    ended: "Đã kết thúc",
    noLimit: "Không giới hạn",
    participants: "{{count}} người tham gia",
    challengeCount: "{{count}} thử thách",
    allActiveChallenges: "Các thử thách đang diễn ra",
    activeChallenge: "Thử thách đang diễn ra",
    allPastChallenges: "Các thử thách đã qua",
    pastChallenge: "Thử thách đã qua",
    noActiveChallenges: "Không tìm thấy thử thách nào",
    noPastChallenges: "Không tìm thấy thử thách nào",
    leaderboard: {
      pastLeaderboardTitle: "Bảng xếp hạng quá khứ",
      pastLeaderboardDesc: "Khám phá kết quả cuối cùng của các thử thách đã đóng",
      winner: "Người chiến thắng",
      updating: "Đang cập nhật...",
      viewLeaderboard: "Xem bảng xếp hạng",
      votingClosed: "Đã đóng bình chọn",
      finalResults: "Kết quả cuối cùng",
      dataFrozenAt: "Dữ liệu được chốt tại thời điểm thử thách kết thúc",
      dataFrozenNotice:
        "Kết quả sẽ không thay đổi ngay cả khi lượt thích thay đổi sau thời điểm này",
      emptyLeaderboard: "Bảng xếp hạng trống",
      ruleShare: "1 lượt chia sẻ = 1 điểm",
      ruleComment: "1 bình luận = 2 điểm",
      ruleLike: "1 lượt thích = 1 điểm",
      ruleView: "1 lượt xem = 1 điểm",
      pointCalculation: "Cách tính điểm",
      prizes: "Giải thưởng",
      prev: "Trước",
      next: "Tiếp",
      title: "Bảng xếp hạng",
      noData: "Không có dữ liệu xếp hạng",
      infoTitle: "Thông tin xếp hạng",
      daysLeft: "Còn {days} ngày {hours} giờ",
      hoursLeft: "Còn {hours} giờ",
      endingSoon: "Sắp kết thúc",
      yourRank: "Thứ hạng của bạn",
      page: "Trang",
      specialScholarship: "Học bổng đặc biệt",
    },
    createOwnReels: "Sáng tạo nội dung của riêng bạn",
    shareKnowledge: "Chia sẻ kiến thức và luyện tập nhập vai cùng cộng đồng Cat Speak.",
    formatLimit: "Định dạng MP4, MOV",
    sizeLimit: "Tối đa 5 phút, 50MB",
    uploadReel: "Đăng tải Reel",
    dragDropVideo: "Kéo thả video của bạn vào đây để tải lên",
    clickToBrowse: "hoặc nhấp để chọn tệp",
    post: "Đăng",
    posting: "Đang đăng...",
    createModal: {
      details: "Chi tiết",
      title: "Tiêu đề",
      titlePlaceholder: "Hãy đặt một tiêu đề thật thu hút...",
      description: "Mô tả",
      descPlaceholder: "Video này nói về điều gì? Thêm thẻ như #catspeak...",
      privacy: "Quyền riêng tư",
      privacyOptions: { Public: "Công khai", FriendsOnly: "Bạn bè", Private: "Chỉ mình tôi" },
      coverThumbnail: "Ảnh bìa",
      extractFrame: "Trích xuất khung hình",
      customImage: "Ảnh tùy chỉnh",
      scrubTimeline: "DUYỆT DÒNG THỜI GIAN",
      quickKeyframes: "Khung hình nhanh",
      extractingKeyframes: "Đang trích xuất...",
      noFrames: "Không có khung hình",
      coverInstruction: "Duyệt dòng thời gian hoặc chọn khung hình nhanh để tự động cập nhật ảnh bìa. Khung hình bạn chọn sẽ được tải lên làm ảnh bìa cho video.",
      uploadCustomCover: "Tải lên ảnh bìa",
      coverFormat: "PNG, JPG tối đa 5MB",
      customImageUploaded: "Đã tải ảnh tùy chỉnh",
      removeImage: "Xóa ảnh",
      preview: "Xem trước",
      videoPlayback: "Phát Video",
      thumbnailPreviewing: "Đang xem trước ảnh bìa",
      untitledReel: "Video chưa có tiêu đề",
      editDetails: "Chỉnh sửa chi tiết",
      livePreview: "Xem trực tiếp",
      uploaded: "Đã tải lên"
    },
    allEvents: "Tất cả sự kiện",
    activeEvent: "Sự kiện đang diễn ra",
    weeklyEvent: "Sự kiện tuần",
    pastEvents: "Sự kiện đã qua",
    endedEvent: "Sự kiện đã kết thúc",
    allActiveChallengesDesc: "Khám phá và tham gia tất cả các thử thách đang diễn ra từ cộng đồng Cat Speak.",
    allPastChallengesDesc: "Xem lại tất cả các thử thách đã kết thúc và những video xuất sắc nhất.",
    joinActiveChallengeDesc: "Hãy tham gia và quay video Reels tương tác của bạn ngay!",
    pastChallengeDesc: "Thử thách này đã kết thúc. Xem các bài nộp xuất sắc.",
    learnMore: "Tìm hiểu thêm",
    votingClosed: "Đã đóng cổng bình chọn",
    entries: "bài dự thi",
    newest: "Mới nhất",
    oldest: "Cũ nhất",
    monthlyCreator: "Nhà sáng tạo của tháng",
    thisWeek: "Tuần này",
    finalRanking: "Bảng chung cuộc",
    noRankings: "Chưa có xếp hạng nào",
    viewFullRanking: "Xem toàn bộ xếp hạng",
    noReelsFound: "Không tìm thấy Thước phim nào",
    trySelectingDifferentTag: "Hãy thử chọn một thẻ khác hoặc quay lại sau.",
    detail: {
      commentsTotal: "bình luận",
      beFirstToComment: "Hãy là người đầu tiên bình luận.",
      signInToComment: "Đăng nhập để bình luận...",
      replyTo: "Trả lời",
      reply: "Trả lời",
      delete: "Xóa",
      deleteConfirm: "Bạn có chắc chắn muốn xóa bình luận này không?",
      comment: "Bình luận",
      send: "Gửi",
      showMore: "Xem thêm",
      showLess: "Thu gọn",
      viewReplies: "Xem phản hồi",
      hideReplies: "Ẩn phản hồi",
      reelNotFound: "Không tìm thấy Reel",
      reelNotFoundDesc: "Reel bạn đang tìm không tồn tại hoặc đã bị xóa.",
      backToReels: "← Quay lại Thước phim",
      viewComments: "Xem bình luận",
      caughtUp: "Bạn đã xem hết",
      watchedAll: "Bạn đã xem tất cả các Thước phim hiện có",
      moreMenu: {
        report: "Báo cáo",
        notInterested: "Không quan tâm",
        copyLink: "Sao chép liên kết",
        saveReel: "Lưu Thước phim",
        addToPlaylist: "Thêm vào danh sách phát",
        aboutAccount: "Về tài khoản này",
        notInterestedSuccess: "Bạn sẽ không thấy nội dung tương tự nữa.",
        copySuccess: "Đã sao chép liên kết vào khay nhớ tạm.",
        addToPlaylistSuccess: "Đã thêm vào danh sách phát.",
        accountIdMissing: "Không tìm thấy ID tài khoản cho Thước phim này.",
        featureNotAvailable: "Tính năng này hiện chưa khả dụng.",
        actionFailed: "Không thể thực hiện hành động này.",
      },
      playlistModal: {
        title: "Lưu vào danh sách phát",
        createNew: "Tạo danh sách phát mới",
        placeholder: "Nhập tên danh sách phát...",
        create: "Tạo",
        creating: "Đang tạo...",
        empty: "Bạn chưa có danh sách phát nào.",
        defaultSave: "Lưu nhanh (Mặc định)",
        itemCount: "{{count}} video",
        itemCountSingular: "1 video",
      },
      reportModal: {
        title: "Báo cáo",
        question: "Tại sao bạn báo cáo Thước phim này?",
        details: "Chi tiết bổ sung (Tùy chọn)",
        placeholder: "Cung cấp thêm ngữ cảnh để giúp chúng tôi hiểu...",
        cancel: "Hủy",
        submit: "Gửi báo cáo",
        submitting: "Đang gửi...",
        selectReasonError: "Vui lòng chọn một lý do.",
        success: "Cảm ơn bạn. Chúng tôi đã nhận được báo cáo của bạn.",
        error: "Gửi báo cáo thất bại. Vui lòng thử lại.",
        reasons: {
          spam: "Spam",
          nudity: "Ảnh khỏa thân hoặc hoạt động tình dục",
          hateSpeech: "Ngôn từ hoặc biểu tượng thù địch",
          violence: "Bạo lực hoặc tổ chức nguy hiểm",
          illegalGoods: "Bán hàng hóa bất hợp pháp hoặc bị quản lý",
          bullying: "Bắt nạt hoặc quấy rối",
          ipViolation: "Vi phạm sở hữu trí tuệ",
          suicide: "Tự tử hoặc tự gây thương tích",
          eatingDisorders: "Rối loạn ăn uống",
          scam: "Lừa đảo hoặc gian lận",
          falseInfo: "Thông tin sai lệch"
        }
      },
      accountModal: {
        reels: "Thước phim",
        likes: "Lượt thích",
        joined: "Đã tham gia",
        reputationScore: "Điểm uy tín:",
        close: "Đóng",
        loadingError: "Không thể tải chi tiết tài khoản.",
      }
    },
    workspace: {
      tabs: {
        myReels: "Reels của tôi",
        playlists: "Danh sách phát",
      },
      totalReels: "Tổng Reels",
      totalViews: "Tổng lượt xem",
      totalLikes: "Tổng lượt thích",
      reelsUploaded: "Đã tải lên {{count}} reels",
      reelUploadedSingular: "Đã tải lên 1 reel",
      noReels: "Chưa có Reels nào",
      noReelsDesc: "Chia sẻ kiến thức, bài hướng dẫn hoặc khoảnh khắc nổi bật bằng cách tải lên video ngắn đầu tiên!",
      uploadFirst: "Tải lên Reel đầu tiên",
      deleteTitle: "Xóa Reel?",
      deleteConfirm: "Bạn có chắc chắn muốn xóa",
      deleteWarning: "Hành động này không thể hoàn tác.",
      deleteBtn: "Xóa",
      cancelBtn: "Hủy",
      deleting: "Đang xóa Reel...",
      deleteSuccess: "Đã xóa Reel thành công!",
      deleteFailed: "Xóa Reel thất bại.",
      moderationStatus: {
        Moderating: "Đang kiểm duyệt",
        PendingReview: "Chờ duyệt",
        Rejected: "Bị từ chối",
        Blocked: "Đã bị chặn",
      },
      moderationHint: {
        Moderating: "Video đang được kiểm tra, thường mất vài phút.",
        PendingReview: "Video cần quản trị viên xem lại trước khi hiển thị.",
        Rejected: "Video vi phạm tiêu chuẩn cộng đồng nên không được hiển thị.",
        Blocked: "Reel đã bị quản trị viên chặn.",
      },
      totalPlaylists: "Tổng danh sách phát",
      totalSaved: "Tổng đã lưu",
      noPlaylists: "Chưa có danh sách phát nào",
      noPlaylistsDesc: "Lưu Reels vào danh sách phát để sắp xếp và xem lại nội dung yêu thích!",
      createFirst: "Tạo danh sách phát đầu tiên",
      createdOn: "Tạo ngày",
      noVideosInPlaylist: "Không có video nào trong danh sách phát này.",
      removingFromPlaylist: "Đang xóa...",
      removedFromPlaylist: "Đã xóa khỏi danh sách phát.",
      removeFailed: "Xóa thất bại.",
    },
  },
  mail: {
    placeholderEmpty: "Hãy là người đầu tiên nói xin chào...",
    placeholder: "Viết câu chuyện của bạn...",
    noStories: "Chưa có câu chuyện nào. Bắt đầu cuộc trò chuyện!",
    yours: "của bạn",
    total: "tổng cộng",
    sortNewest: "Mới nhất",
    sortOldest: "Cũ nhất",
    displayGrid: "Lưới",
    displayFloat: "Trôi nổi",
    emptyInputError: "Vui lòng nhập nội dung trước khi gửi.",
  },
  newsLoginPrompt:
    "Vui lòng đăng nhập để xem những bài viết mới nhất từ cộng đồng.",
  sidebar: {
    news: "Bản tin Cat Speak",
    discover: "Khám phá thế giới",
    reels: "Thước phim",
    letters: "Thư",
    calendar: "Sự kiện",
    settings: "Cài đặt",
    report: "Nhật ký báo cáo",
    help: "Trợ giúp",
    feedback: "Gửi ý kiến phản hồi",
    menu: "Menu",
  },
  created: "Đã tạo",
  expiresIn: "Hết hạn trong",
  story: "Câu chuyện",
};
