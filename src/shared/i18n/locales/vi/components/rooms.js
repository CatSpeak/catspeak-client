export default {
  language: {
    vi: "Tiếng Việt",
    en: "English",
    zh: "China",
  },
  welcome: {
    greeting: "Chào {{name}},",
    friend: "Bạn",
    title: "Happy Halloween",
    description: {
      part1: "Halloween về danh nghĩa là một ",
      highlight1: "lễ hội Kitô giáo",
      part2:
        " tôn vinh linh hồn của các vị thánh và những linh hồn khác đã được ban phước. Nó bắt nguồn từ một ",
      highlight2: "lễ hội Celtic cổ đại",
      part3:
        " của người chết đánh dấu sự kết thúc chính thức của mùa trồng trọt.",
    },
    trickOrTreat: "Trick or Treat",
    allowConnect: "Cho phép kết nối với bạn bè quốc tế",
  },
  specialNames: {
    doYouLikeToGoOut: "Bạn thích đi chơi không?",
    hskExamPrep: "Phòng luyện thi HSK",
    doYouHavePets: "Bạn có nuôi thú cưng không?",
    schoolLife: "Cuộc sống ở trường",
    whatILike: "Điều tôi thích!",
    doYouLikeReading: "Bạn có thích đọc sách?",
    aboutLove: "Chuyện về tình yêu",
    genZChineseCulture: "Văn hóa Gen Z Trung Quốc",
    famousPlaces: "Các điểm du lịch nổi tiếng",
    yourCountryCulture: "Văn hóa đất nước",
  },
  heroCarousel: {
    comingSoonTitle: "Sắp ra mắt",
    slides: [],
  },
  searchPlaceholder: "Tìm kiếm theo tên phòng, chủ đề...",
  searchBtn: "Tìm kiếm",
  sortTooltip: "Sắp xếp",
  sortTitle: "Sắp xếp danh sách",
  sortFieldLabel: "Thuộc tính sắp xếp",
  sortOrderLabel: "Thứ tự sắp xếp",
  sortAsc: "Tăng dần",
  sortDesc: "Giảm dần",
  sortReset: "Đặt lại",
  sortOptions: {
    name: "Tên phòng",
    createdAt: "Ngày tạo",
    participants: "Số người tham gia",
    duration: "Thời lượng",
  },
  workspace: {
    customRooms: "Phòng tùy chỉnh",
    bookmarkedRooms: "Phòng đã lưu",
  },
  noBookmarkedRooms: "Chưa có phòng nào được lưu",
  noBookmarkedRoomsDesc: "Lưu các phòng yêu thích để truy cập nhanh chóng bất cứ lúc nào!",
  noSearchResults: "Không tìm thấy kết quả phù hợp",
  noSearchResultsDesc: "Thử thay đổi từ khóa tìm kiếm hoặc điều chỉnh bộ lọc của bạn.",
  loadError: "Không thể tải danh sách phòng. Vui lòng thử lại.",
  retry: "Thử lại",
  bookmarkUpdated: "Đã cập nhật danh sách phòng đã lưu",
  bookmarkUpdateError: "Không thể thay đổi lưu phòng",
  pagination: {
    prev: "Trước",
    next: "Sau",
    pageOf: "Trang {page} / {totalPages}",
  },
  filters: {
    title: "Phòng",
    filterModalTitle: "Bộ lọc phòng",
    filterTooltip: "Bộ lọc",
    roomTypeLabel: "Loại phòng",
    visibilityLabel: "Hiển thị",
    activityLabel: "Hoạt động",
    languageLabel: "Ngôn ngữ",
    clearAll: "Xóa tất cả",
    room: "phòng",
    totalSuffix: "phòng",
    breadcrumb: "Phòng",
    search: "Tìm kiếm",
    searchPlaceholder: "Tìm kiếm",
    topicsAndLevels: "Chủ đề | Trình độ",
    levelsHeading: "Trình độ",
    topicsHeading: "Chủ đề",
    roomTypes: {
      All: "Tất cả",
      Temporary: "Tạm thời",
      Custom: "Tùy chỉnh",
      twoToFive: "2 : 5",
      saved: "Đã lưu",
      forum: "Diễn đàn",
    },
    visibilities: {
      All: "Tất cả",
      Public: "Công khai",
      Private: "Riêng tư",
    },
    activities: {
      All: "Tất cả",
      InUse: "Đang diễn ra",
      Empty: "Đang trống",
    },
    languages: {
      All: "Tất cả",
      English: "Tiếng Anh",
      Chinese: "Tiếng Trung",
      Japanese: "Tiếng Nhật",
      Vietnamese: "Tiếng Việt",
    },
    topics: {
      history: "Lịch sử",
      science: "Khoa học",
      philosophy: "Triết học",
      psychology: "Tâm lý học",
      politics: "Chính trị",
      space: "Không gian",
      movies: "Phim ảnh",
      music: "Âm nhạc",
      art: "Nghệ thuật",
      fashion: "Thời trang",
      culture: "Văn hóa",
      books: "Sách",
      travel: "Du lịch",
      food: "Ẩm thực",
      nature: "Thiên nhiên",
      relationships: "Mối quan hệ",
      sports: "Thể thao",
      finance: "Tài chính",
      startups: "Khởi nghiệp",
      productivity: "Năng suất",
      other: "Khác",
    },
    levels: {
      a1: "A1",
      a2: "A2",
      b1: "B1",
      b2: "B2",
      c1: "C1",
      c2: "C2",
      hsk1: "HSK 1",
      hsk2: "HSK 2",
      hsk3: "HSK 3",
      hsk4: "HSK 4",
      hsk5: "HSK 5",
      hsk6: "HSK 6",
      n1: "N1",
      n2: "N2",
      n3: "N3",
      n4: "N4",
      n5: "N5",
      beginner: "Người mới bắt đầu",
      intermediate: "Trung cấp",
      advanced: "Nâng cao",
    },
    levelLabel: "Trình độ",
    categories: {
      random: "Kết nối ngẫu hứng",
      exam: "Luyện thi",
      friends: "Kết bạn bốn phương",
      trending: "Trending",
      practice: "Luyện thi",
      others: "Khác",
      knowledge: "Học điều mới",
      culture: "Văn hóa & Sáng tạo",
      lifestyle: "Phong cách sống",
      growth: "Phát triển bản thân",
      other: "Chủ đề tự do",
    },
    seeMore: "Xem thêm",
    noRoomsFound: "Không tìm thấy phòng nào cho các danh mục này",
    noRoomsFoundCategory:
      "Hiện tại chưa có phòng nào ở đây! Hãy khám phá các danh mục khác nhé.",
    clear: "Xóa",
    apply: "Áp dụng",
  },
  tabs: {
    communicate: "Giao tiếp",
    teaching: "Giảng dạy",
    group: "Nhóm",
    class: "Lớp học",
    community: "Cộng đồng",
    teachers: "Giảng viên",
    forum: "Diễn đàn",
  },
  people: "người",
  badge: {
    live: "ĐANG DIỄN RA",
  },
  joinRoom: {
    title: "Tham gia phòng",
  },
  createRoom: {
    title: "Tạo phòng",
    typeGroup: "Nhóm",
    typeCustom: "Tùy chỉnh",
    roomTypeLabel: "Loại phòng",
    typeGroupDesc: "Phòng tạm thời, tối đa 5 người",
    typeCustomDesc: "Phòng vĩnh viễn, tối đa {max} người",
    formHint: "Nhập các thông tin cơ bản để tạo phòng",
    topicSelected: "Đã chọn {{count}}/{{max}}",
    namePlaceholder: "ví dụ: Luyện tập vui vẻ",
    nameRequired: "Vui lòng nhập tên phòng",
    topicsLabel: "Chủ đề",
    nameLabel: "Tên phòng",
    thumbnailLabel: "Ảnh bìa phòng",
    uploadImage: "Tải ảnh lên",
    noFileChosen: "Chưa chọn tệp nào",
    requiredLevel: "Trình độ yêu cầu",
    privateRoom: "Phòng riêng tư",
    passwordLabel: "Mật khẩu",
    passwordPlaceholder: "Nhập mật khẩu phòng",
    passwordRequiredMessage: "Phòng riêng tư yêu cầu phải có mật khẩu.",
    cancel: "Hủy",
    join: "Tham gia phòng",
    joining: "Đang tham gia...",
    create: "Tạo phòng",
    creating: "Đang tạo...",
    validation: {
      required: "Vui lòng nhập tên phòng",
      minLength: "Sử dụng ít nhất 3 ký tự",
    },
    topics: {
      other: "Khác",
      history: "Lịch sử",
      science: "Khoa học",
      philosophy: "Triết học",
      psychology: "Tâm lý học",
      politics: "Chính trị",
      space: "Không gian",
      movies: "Phim",
      music: "Âm nhạc",
      art: "Nghệ thuật",
      fashion: "Thời trang",
      culture: "Văn hóa",
      books: "Sách",
      travel: "Du lịch",
      food: "Ẩm thực",
      nature: "Thiên nhiên",
      relationships: "Mối quan hệ",
      sports: "Thể thao",
      finance: "Tài chính",
      startups: "Khởi nghiệp",
      productivity: "Năng suất",
    },
    topicLimit: "Chọn tối đa 3 chủ đề",
  },
  chatBox: {
    title: "Tin nhắn phòng",
    empty: "Chưa có tin nhắn nào",
    you: "Bạn",
    inputPlaceholder: "Nhập gì đó...",
    connectingPlaceholder: "Đang kết nối...",
    showSystemMessages: "Hiển thị tin nhắn Cat Speak gợi ý",
    aiAssistant: "Trợ lý Cat Speak",
    aiEmptyText: "Nhập câu hỏi của bạn bên dưới để hỏi Trợ lý Cat Speak.",
    publicAiPlaceholder: "Hỏi công khai",
    privateAiPlaceholder: "Hỏi riêng tư",
    publicPrompt: "Hỏi công khai",
    privatePrompt: "Hỏi riêng tư",
    systemName: "Cat Speak gợi ý",
    systemMessages: "Tin nhắn hệ thống",
    systemMessagesDesc: "Hiển thị các tin nhắn hệ thống tự động",
    replyingTo: "Đang trả lời",
    reply: "Trả lời",
    aiSuggestion: "Gợi ý từ AI",
    aiMeetingSuggestionLabel: "GỢI Ý CÂU CHO BUỔI HỌC",
    aiGreetingIntro: "Xin chào! Dưới đây là các câu gợi ý theo chủ đề buổi học hôm nay để bạn luyện nói hoặc hỏi AI từ vựng liên quan:",
    aiAskVocabularyTooltip: "Click để hỏi AI từ vựng cho câu này",
    aiAskVocabularyPrompt: 'Gợi ý cho tôi các từ vựng phù hợp thường được sử dụng cho câu: "{{sentence}}"',
    aiLoadMoreSuggestions: "+ Gợi ý thêm",
    aiTopicLabel: "Chủ đề:",
    aiPublicWarningBadge: "🌐 Công khai với phòng",
    aiPlaceholderDynamic: "Hỏi gì cũng được về {language}...",
    aiRetry: "Thử lại",
    aiErrorTitle: "Không thể kết nối trợ lý",
    aiErrorResponse: "Timeout hoặc lỗi mạng khi trợ lý AI xử lý câu hỏi. Vui lòng thử lại.",
    aiQuotaExceeded: "Bạn đã đạt giới hạn sử dụng AI hôm nay. Vui lòng nâng cấp gói để tiếp tục!",
    aiFollowUpHeader: "GỢI Ý CÂU HỎI TIẾP THEO CHO AI:",
    aiTypingResponse: "AI đang soạn câu trả lời...",
    privateModeLabel: "Riêng tư",
    publicModeLabel: "Công khai",
    mentionHeader: "Thành viên trong phòng",
    mentionHint: "↑↓ di chuyển, Enter chọn",
    mentionNotFound: "Không tìm thấy thành viên phù hợp",
    mentionAllDesc: "Tất cả thành viên trong phòng",
    chatRestrictedPlaceholder: "Bạn đang bị hạn chế gửi tin nhắn chat",
    chatRestrictedReason:
      "Host đã hạn chế quyền gửi tin nhắn chat của bạn.",
  },
  sessionActions: {
    connect11: "Kết nối 1:1",
    connect25: "Kết nối 2:5",
    yourAI: "1:1 với AI",
    customRoom: "Tạo phòng",
    aiChat: "Trò chuyện với AI",
  },
  customRooms: {
    expiredWarning: "Đã hết hạn — sẽ được xóa",
    assignedCoHost: "Đã phân công Co-host",
    noCoHost: "Chưa có Co-host",
    title: "Phòng Tùy Chỉnh Pro",
    myRoomsTitle: "Phòng Của Tôi",
    createTitle: "Tạo Phòng Tùy Chỉnh",
    editTitle: "Chỉnh Sửa Phòng",
    quota: "{{used}}/{{max}} phòng đã dùng",
    quotaFull: "Bạn đã đạt giới hạn tối đa {{max}} phòng",
    capacity: "Sức chứa: 100 người",
    persistent: "Phòng Vĩnh Viễn",
    roomName: "Tên Phòng",
    roomNamePlaceholder: "VD: Nhóm Học Tập",
    description: "Mô tả",
    descriptionPlaceholder: "Phòng này về chủ đề gì?",
    create: "Tạo phòng",
    creating: "Đang tạo...",
    save: "Lưu Thay Đổi",
    saving: "Đang lưu...",
    delete: "Xóa",
    deleteConfirmTitle: "Xóa phòng?",
    deleteConfirmMessage: 'Bạn có chắc chắn muốn xóa "{{name}}"? Hành động này không thể hoàn tác.',
    deleteConfirm: "Bạn có chắc chắn muốn xóa phòng này?",
    deleteSuccess: "Đã xóa phòng thành công",
    createSuccess: "Đã tạo phòng tùy chỉnh thành công",
    updateSuccess: "Đã cập nhật phòng thành công",
    noRooms: "Bạn chưa tạo phòng tùy chỉnh nào",
    noRoomsSubtext:
      "Tạo phòng vĩnh viễn đầu tiên với tối đa 100 người tham gia!",
    join: "Tham gia",
    copyLink: "Sao chép liên kết",
    linkCopied: "Đã sao chép!",
    tabCreate: "Tạo mới",
    tabMyRooms: "Phòng của tôi",
    proBadge: "PRO",
    maxRoomsReached: "Đã đạt giới hạn phòng",
    manageRooms: "Quản lý phòng",
    nameRequired: "Vui lòng nhập tên phòng",
    privateHint: "Yêu cầu mật khẩu để tham gia phòng này.",
    publicHint: "Không yêu cầu mật khẩu. Mọi người đều có thể tham gia.",
    private: "Riêng tư",
    passwordRequired: "Yêu cầu mật khẩu",
    passwordLabel: "Mật khẩu (Tùy chọn)",
    passwordPlaceholder: "Nhập mật khẩu phòng",
    roomThumbnail: "Ảnh bìa phòng",
    uploadThumbnail: "Tải ảnh lên",
    dragDropHint: "Kéo & thả ảnh bìa vào đây",
    imageHint: "PNG, JPG, WEBP tối đa 5MB",
    change: "Thay đổi",
    remove: "Xóa",
    maxParticipantsLabel: "Số người tối đa",
    maxParticipantsLimit: "Tối đa: {max} người",
    people: "người",
    unlimited: "Vĩnh viễn",
    roomLifetime: "Vòng đời phòng",
    roomLifetimeForever: "Mãi mãi",
    languageLabel: "Ngôn ngữ",
    optional: "Tùy chọn",
    nameMaxLength: "Tên phòng tối đa 50 ký tự",
    formHint: "Thiết lập thông tin cho phòng tùy chỉnh của bạn",
    sectionBasic: "Thông tin cơ bản",
    sectionSettings: "Cài đặt phòng",
  },
  aiSettings: {
    title: "Cài đặt phiên AI",
    language: "Ngôn ngữ",
    learningLanguage: "Ngôn ngữ học",
    supportLanguage: "Ngôn ngữ hỗ trợ",
    supportNone: "Không có",
    english: "Tiếng Anh",
    chinese: "Tiếng Trung",
    vietnamese: "Tiếng Việt",
    japanese: "Tiếng Nhật",
    speed: "Tốc độ nói",
    speedSlow: "Chậm",
    speedNormal: "Bình thường",
    speedFast: "Nhanh",
    proficiencyLabel: "Trình độ",
    proficiencyOptional: "Không bắt buộc",
    voice: "Giọng nói",
    voiceFemale: "Nữ",
    voiceMale: "Nam",
    voiceUnavailable: "Giọng {{voice}} không khả dụng cho {{language}}",
    cancel: "Hủy",
    start: "Bắt đầu phiên",
  },
  queue: {
    title: "Tìm đối tác",
    findMatch: "Tìm ngay",
    cancel: "Hủy",
    connectingToQueue: "Đang kết nối vào hàng chờ...",
    matchFound: "Đã tìm thấy match!",
    waitingForPairing: "Đang chờ ghép đôi...",
    error: "Lỗi: {{msg}}",
    socketConnected: "Đã kết nối Socket",
    connecting: "Đang kết nối...",
    findingMatch:
      "Chúng mình đang tìm người phù hợp nhất với trình độ của bạn.",
    yourPosition: "Vị trí của bạn:",
    cancelSearch: "Hủy tìm kiếm",
    status: "TRẠNG THÁI",
    position: "VỊ TRÍ",
    connected: "Đã kết nối",
    connectingStatus: "Đang kết nối",
  },
  waitingScreen: {
    backToCommunity: "Quay lại",
    readyToJoin: "Sẵn sàng tham gia?",
    micOff: "Tắt mic",
    joinNow: "Tham gia ngay",
    copyLink: "Sao chép liên kết",
    linkCopied: "Đã sao chép!",
    joinedAs: "Tham gia với tên",
    joinedAsNickname: "Tham gia với biệt danh",
    editProfile: "Chỉnh sửa hồ sơ",
    saveName: "Lưu",
    cancelEdit: "Hủy",
    updateNameSuccess: "Cập nhật biệt danh thành công",
    updateNameError: "Cập nhật biệt danh thất bại",
    editName: "Sửa Tên",
    namePlaceholder: "Nhập biệt danh",
    noOneHere: "Chưa có ai ở đây cả.",
    isHere: "đang ở đây",
    loading: "Đang tải...",
    loadingRoom: "Đang tải phòng...",
    roomNotFound: "Không tìm thấy phòng",
    roomNotFoundSubtext:
      "Phòng bạn đang tìm không tồn tại hoặc đã bị đóng. Hãy khám phá những phòng khác đang diễn ra sôi nổi nhé!",
    roomFull: "Phòng đã đủ người (tối đa 5).",
    joinError: "Có lỗi xảy ra khi tham gia phòng.",
    createSessionError: "Không thể tạo phiên. Vui lòng thử lại.",
    cameraAccessError:
      "Không thể truy cập camera. Kiểm tra quyền hoặc thiết bị đang được sử dụng.",
    micAccessError:
      "Không thể truy cập micro. Kiểm tra quyền hoặc thiết bị đang được sử dụng.",
    micNotFound: "Không tìm thấy phần cứng micro.",
    cameraNotFound: "Không tìm thấy phần cứng camera.",
    micInUse:
      "Micro đang được sử dụng bởi ứng dụng khác hoặc không thể khởi động.",
    cameraInUse:
      "Camera đang được sử dụng bởi ứng dụng khác hoặc không thể khởi động.",
    micPermissionDenied:
      "Quyền truy cập micro bị từ chối. Vui lòng cho phép trong cài đặt.",
    cameraPermissionDenied:
      "Quyền truy cập camera bị từ chối. Vui lòng cho phép trong cài đặt.",
    webViewWarning:
      "Bạn đang mở trong ứng dụng (Zalo/FB/Messenger). Vui lòng nhấn '...' chọn 'Mở bằng Safari' để dùng Micro và Camera.",
    iosPermissionTip:
      "Microphone đã bị trình duyệt chặn. Vui lòng nhấn vào Cài đặt trang web ở thanh địa chỉ để mở Micro.",
    deviceSettings: "Cài đặt",
    deviceSettingsTab: "Cài đặt thiết bị",
    generalTab: "Chung",
    myPreferences: "Tùy chọn của tôi",
    bannedListTab: "Danh sách bị cấm",
    selectMicrophone: "Chọn Micro",
    selectSpeaker: "Chọn Loa",
    systemDefaultSpeaker: "Loa Mặc Định Hệ Thống",
    systemDefault: "Mặc định hệ thống",
    speakerNotSupported: "Trình duyệt của bạn không hỗ trợ chọn loa.",
    selectCamera: "Chọn Camera",
    testMic: "Kiểm tra mic",
    stopTest: "Dừng kiểm tra",
    micTestHint: "Nhấn Kiểm tra mic và nói vài câu để xem mức âm thanh.",
    micTestListening: "Hãy nói vài câu...",
    micTestDetected: "Đã nhận âm thanh",
    unknownDevice: "Thiết bị không xác định",
  },
  videoCall: {
    inviteParticipant: "Mời tham gia phòng",
    sendInvite: "Gửi lời mời",
    inviteDescription: "Nhập địa chỉ email của người bạn muốn mời vào phòng này. Họ sẽ nhận được thông báo kèm liên kết để tham gia.",
    changeLayout: {
      title: "Điều chỉnh chế độ xem",
      auto: "Tự động (linh động)",
      grid: "Lưới",
      spotlight: "Tiêu điểm",
      sidebar: "Thanh bên",
      tilesLabel: "Số ô",
      tilesDescription: "Số ô tối đa được hiển thị, tùy vào kích thước cửa sổ.",
      hideNonVideo: "Ẩn ô không có video",
    },
    remainingTime: "Thời gian còn lại",
    roomClosingTitle: "Phòng sắp đóng",
    roomClosingSubtitle:
      "Phiên này sẽ kết thúc trong {{remainingSeconds}} giây",
    roomClosingSubtitleStatic: "Phiên kết nối này sắp kết thúc.",
    roomClosingDescription:
      "Vui lòng kết thúc cuộc trò chuyện. Phòng sẽ tự động đóng khi thời gian kết thúc.",
    screenShareLabel: "Màn hình của {{name}}",
    screenShareLabelYou: "Màn hình của {{name}} (Bạn)",
    screenShare: {
      takeoverTitle: "Chia sẻ màn hình thay thế?",
      takeoverConfirm: "Chia sẻ thay thế",
      takeoverMessage: "{{name}} đang chia sẻ màn hình. Bắt đầu chia sẻ màn hình mới sẽ dừng phần trình bày của {{name}}.",
      hostProtected: "Chủ phòng đang thuyết trình. Bạn không thể ghi đè màn hình của chủ phòng.",
    },
    moreOptions: "Tùy chọn khác",
    watchTogether: {
      toggleButton: "Xem video chung",
      title: "Xem video chung",
      inputLabel: "Dán link YouTube",
      startButton: "Phát video cho cả phòng",
      starting: "Đang phát...",
      stopButton: "Dừng video",
      stopping: "Đang dừng...",
      invalidUrl: "Vui lòng nhập một link YouTube hợp lệ.",
      hostOnly: "Chỉ chủ phòng có thể phát video chung.",
      startSuccess: "Đang phát video chung...",
      stopSuccess: "Đã dừng video chung.",
      startError: "Không thể phát video. Vui lòng thử lại.",
      stopError: "Không thể dừng video.",
      hostWatching: "Chủ phòng đang phát video chung.",
      notPlaying: "Chưa có video nào được phát trong phòng.",
      ended: "Đã kết thúc.",
      tapToSync: "Nhấn để đồng bộ",
      embedBlocked: "Video này không cho phép phát chung.",
      syncError: "Không thể tải video. Kiểm tra kết nối rồi thử lại.",
      switchButton: "Đổi sang video này",
      switchLabel: "Dán link YouTube mới để đổi video",
      nowPlaying: "Đang phát",
      captionsOn: "Bật phụ đề",
      captionsOff: "Tắt phụ đề",
      noCaptions: "Video này không có phụ đề",
    },
    subtitles: "Phụ đề (CC)",
    reconnect: "Khắc phục sự cố kết nối",
    troubleshoot: {
      havingIssues: "Gặp sự cố?",
      description:
        "Nếu bạn gặp tình trạng giật lag, video bị đóng băng hoặc mất âm thanh, hãy thử buộc kết nối lại. Cuộc gọi của bạn sẽ tạm dừng một chút trong khi chúng tôi thiết lập lại kết nối.",
      forceReconnect: "Buộc kết nối lại",
    },
    general: {
      receiveSystemMsgs: "Nhận thông báo tin nhắn hệ thống",
      receiveSystemMsgsDesc:
        "Hiển thị các thông báo tự động từ hệ thống trong khi cuộc họp đang diễn ra.",
      showAiSuggestions: "Hiển thị tin nhắn Cat Speak gợi ý",
      showAiSuggestionsDesc:
        "Hiển thị các thông báo tin nhắn gợi ý tự động từ Trợ lý Cat Speak trong khung chat.",
      joinLeaveSound: "Âm thanh khi người dùng vào / ra phòng",
      joinLeaveSoundDesc:
        "Phát chuông thông báo âm thanh khi có thành viên mới vào hoặc rời khỏi cuộc họp.",
      allowMemberRecording: "Cho phép thành viên ghi hình cuộc họp",
      allowMemberRecordingDesc:
        "Khi tắt, thành viên không thể dùng tính năng record nền tảng và video sẽ bị che đen nếu quay bằng ứng dụng bên ngoài (âm thanh vẫn giữ nguyên).",
      allowMemberPrivateAi: "Cho phép thành viên sử dụng AI Chat riêng tư",
      allowMemberPrivateAiDesc:
        "Khi tắt, thành viên chỉ có thể sử dụng AI Chat công khai trong phòng họp, không thể trò chuyện riêng với AI.",
      allowGame: "Cho phép trò chơi trong phòng",
      allowGameDesc:
        "Khi tắt, thành viên không thể bắt đầu ván game mới. Ván đang chạy vẫn tiếp tục.",
      allowHighQuality: "Chế độ chất lượng cao (720p)",
      allowHighQualityDesc: "Khi bật, người tham gia publish camera lên 720p. Người ở nước ngoài vẫn chỉ nhận tối đa 2 tile video.",
      privateAiDisabledByHost: "Host đã tắt quyền sử dụng AI Chat riêng tư đối với thành viên.",
    },
    copyLink: "Sao chép liên kết phòng",
    linkCopied: "Đã sao chép!",
    leaveCall: "Rời phòng",
    leaveCallConfirmText: "Bạn có chắc chắn muốn rời khỏi cuộc gọi này không?",
    pictureInPicture: "Hình trong Hình (PiP)",
    controls: {
      micOn: "Bật micro",
      micOff: "Tắt micro",
      camOn: "Bật camera",
      playGames: "Trò chơi",
      camOff: "Tắt camera",
      shareOn: "Chia sẻ màn hình",
      shareOff: "Dừng chia sẻ",
      recordOn: "Bắt đầu ghi",
      recordOff: "Dừng ghi",
      participants: "Người tham gia",
      speakingTimeBalance: "Cân bằng thời lượng nói",
      chat: "Trò chuyện",
      captionsOn: "Bật phụ đề",
      captionsOff: "Tắt phụ đề",
      detectLanguage: "Phát hiện ngôn ngữ",
      selectSubtitleLanguage: "Chọn ngôn ngữ phụ đề",
      back: "Quay lại",
    },
    speakingTimeBalance: {
      title: "Speaking Time Balance",
      updatedSecondsAgo: "Cập nhật {seconds} giây trước",
      loading: "Đang tải...",
      loadingData: "Đang tải dữ liệu",
      teacherTalk: "Teacher Talk",
      teacher: "GV",
      students: "HV",
      expected: "Exp",
      colDuration: "Thời lượng",
      colName: "Tên",
      colPercent: "%",
      roomStudents: "HV phòng ({count})",
      normal: "Bình thường",
      attention: "Cần chú ý",
      tooLow: "Quá thấp",
      noDataTitle: "Chưa ai phát biểu",
      noDataSubtitle: "Buổi học vừa bắt đầu",
      errorTitle: "Lỗi thu thập dữ liệu",
      errorSubtitle: "Buổi học vẫn tiếp tục",
      errorCode: "E-stb-001 · Tự phục hồi",
      errorFooter: "Hệ thống tự lấy lại dữ liệu khi kết nối phục hồi",
      unableToLoad: "Không thể tải",
      studentWidget: {
        meMainRoom: "Tôi: Main Room",
        meBreakout: "Tôi: {name}",
        speakingPercent: "% phát biểu",
        cumulativeDuration: "thời lượng nói",
        personalMetrics: "Chỉ số bản thân",
        errorSpeakingPercent: "% Pb",
        errorDuration: "Thời lượng",
        errorCannotGet: "Không thể lấy dữ liệu",
      },
    },
    subtitleWaiting: "Đang đợi phụ đề...",
    speechWaiting: "Đang đợi giọng nói...",
    displayLanguage: "Ngôn ngữ hiển thị",
    fontSize: "Cỡ chữ",
    fontSizes: {
      default: "Mặc định",
      xs: "Rất nhỏ",
      sm: "Nhỏ",
      md: "Trung bình",
      lg: "Lớn",
      xl: "Rất lớn",
      "2xl": "Lớn nhất",
    },
    subtitleLanguages: {
      en: "Tiếng Anh",
      vi: "Tiếng Việt",
      zh: "Tiếng Trung",
      ja: "Tiếng Nhật",
    },
    changeAvatar: "Thay đổi ảnh đại diện",
    backgroundsAndEffects: "Nền và hiệu ứng",
    tabBackgrounds: "Phông nền",
    applyVisualEffects: "Thay đổi phông nền",
    cameraOffWarning:
      "Máy ảnh của bạn đang tắt. Việc chọn một hiệu ứng sẽ bật nó lên.",
    recording: {
      stopTitle: "Dừng ghi hình",
      stopDescription:
        "Bản ghi sẽ được lưu tại <strong>Nhà của bạn (My Workspace)</strong>.",
      continueBtn: "Tiếp tục ghi",
      confirmBtn: "Xác nhận",
      backBtn: "Quay lại"
    },
    provider: {
      connecting: "Đang kết nối...",
      reconnecting: "Đang kết nối lại...",
      failedToLoad: "Không thể tải phiên",
      unknownError: "Đã xảy ra lỗi không xác định",
      retry: "Thử lại",
      switchTitle: "Chuyển cuộc gọi?",
      switchMessage:
        "Bạn đang trong một cuộc gọi khác. Bạn có muốn rời khỏi cuộc gọi hiện tại và tham gia cuộc gọi mới này không?",
      switchConfirm: "Rời và tiếp tục",
      switchCancel: "Ở lại cuộc gọi hiện tại",
    },
    error: {
      micPermission:
        "Đã xảy ra lỗi khi tạo track âm thanh. Không thể khởi tạo nguồn âm thanh. Vui lòng kiểm tra cài đặt quyền âm thanh của trình duyệt.",
      camPermission:
        "Đã xảy ra lỗi khi tạo track video. Không thể khởi tạo nguồn video. Vui lòng kiểm tra cài đặt quyền video của trình duyệt.",
      toggleMic: "Không thể bật/tắt micro",
      toggleCam: "Không thể bật/tắt camera",
    },
    participantList: {
      title: "Người tham gia",
      membersTab: "Thành viên",
      loading: "Đang tải...",
      quickActions: "Thao tác nhanh",
      management: "Tùy chọn quản lý",
      settingsTab: "Cài đặt",
      roomPoliciesTitle: "Quyền & Cài đặt phòng",
      back: "Quay lại",
      searchPlaceholder: "Tìm theo tên...",
      noSearchResults: "Không tìm thấy thành viên nào.",
      raisedHandsSection: "Đang giơ tay",
      membersSection: "Thành viên trong phòng",
      advancedActions: "Thao tác nâng cao",
      dangerZone: "Khu vực nguy hiểm",
      endLiveDesc: "Đóng phiên họp trực tiếp cho toàn bộ học viên và người tham gia.",
      endLiveDescRoom: "Đóng phòng và kết thúc phiên hoạt động cho toàn bộ thành viên.",
      restrictVoiceAllDesc: "Tắt mic và chặn tất cả người tham gia tự bật lại mic.",
      restrictVoiceAllDescRoom: "Tắt mic và chặn tất cả thành viên tự bật lại mic.",
      bannedTabShort: "Bị cấm",
      groupStudentPermissions: "Quyền học viên",
      groupStudentPermissionsRoom: "Quyền thành viên",
      groupRoomSecurity: "Phòng & ghi hình",
      forbiddenGame: "Bạn không có quyền thay đổi chính sách trò chơi.",
      forbiddenHighQuality: "Chỉ chủ phòng mới có quyền bật/tắt chế độ chất lượng cao.",
      forbiddenBannedList: "Bạn không có quyền xem danh sách bị cấm của phòng này.",
      you: "Bạn",
      guest: "Khách",
      youSuffix: "(Bạn)",
      micOn: "Mic đang bật",
      micOff: "Mic đang tắt",
      camOn: "Camera đang bật",
      camOff: "Camera đang tắt",
      volume: "Âm lượng",
      unmute: "Bật tiếng",
      mute: "Tắt tiếng",
      adjustVolume: "Điều chỉnh âm lượng",
      options: "Tùy chọn",
      words: "từ",
      wpm: "WPM",
      speakingTime: "Thời gian nói",
      muteAll: "Tắt mic tất cả",
      lowerAllHands: "Hạ tay tất cả",
      muteCam: "Tắt camera",
      unmuteCam: "Bật camera giùm",
      stopScreenShare: "Dừng chia sẻ màn hình",
      kick: "Mời ra khỏi phòng",
      ban: "Xóa & Cấm vào lại",
      confirmMuteAllTitle: "Tắt tất cả mic",
      confirmKickTitle: "Mời ra khỏi phòng",
      confirmBanTitle: "Xóa & Cấm vào lại",
      confirmMuteAll: "Bạn có chắc chắn muốn tắt tiếng tất cả thành viên trong phòng?",
      confirmKick: "Bạn có chắc chắn muốn đưa {name} ra khỏi phòng?",
      confirmBan: "Bạn có chắc chắn muốn Ban {name} khỏi phòng?",
      restrictChat: "Hạn chế chat",
      unrestrictChat: "Gỡ hạn chế chat",
      restrictVoice: "Hạn chế voice",
      unrestrictVoice: "Gỡ hạn chế voice",
      restrictVoiceAll: "Chặn mic tất cả",
      restrictVoiceAllRoom: "Chặn mic tất cả",
      confirmRestrictChatTitle: "Hạn chế chat",
      confirmRestrictChat:
        "Bạn có chắc chắn muốn hạn chế quyền gửi tin nhắn của {name}?",
      confirmUnrestrictChatTitle: "Gỡ hạn chế chat",
      confirmUnrestrictChat:
        "Bạn có chắc chắn muốn gỡ hạn chế quyền gửi tin nhắn của {name}?",
      confirmRestrictVoiceTitle: "Hạn chế voice",
      confirmRestrictVoice:
        "Bạn có chắc chắn muốn hạn chế quyền nói của {name}?",
      confirmUnrestrictVoiceTitle: "Gỡ hạn chế voice",
      confirmUnrestrictVoice:
        "Bạn có chắc chắn muốn gỡ hạn chế quyền nói của {name}?",
      confirmRestrictVoiceAllTitle: "Cấm voice tất cả",
      confirmRestrictVoiceAllTitleRoom: "Tắt mic tất cả thành viên",
      confirmRestrictVoiceAll:
        "Bạn có chắc muốn hạn chế quyền nói của {count} thành viên trong phòng? Họ sẽ không thể tự bật mic cho đến khi được gỡ.",
      confirmRestrictVoiceAllRoom:
        "Bạn có chắc muốn tắt mic của {count} thành viên trong phòng? Họ sẽ không thể tự bật mic cho đến khi được gỡ.",
      successRestrictChat: "Đã hạn chế chat",
      successUnrestrictChat: "Đã gỡ hạn chế chat",
      successRestrictVoice: "Đã hạn chế voice",
      successUnrestrictVoice: "Đã gỡ hạn chế voice",
      successRestrictVoiceAll: "Đã hạn chế voice {count} thành viên",
      successRestrictVoiceAllRoom: "Đã tắt mic {count} thành viên",
      participantNotFound:
        "Người tham gia đã rời phòng. Đang làm mới danh sách.",
      alreadyRestricted: "Đã ở trạng thái đó.",
      noActiveSession: "Không có phiên live đang diễn ra.",
      alreadyVoiceRestricted: "Đã ở trạng thái hạn chế.",
      forbiddenRestrictVoice: "Bạn không có quyền hạn chế voice.",
      forbiddenModerate: "Bạn không có quyền thực hiện thao tác này.",
      hostMutedMic: "Host đã tắt mic của bạn.",
      hostMutedCam: "Host đã tắt camera của bạn.",
      hostUnmutedMic: "Host đã bật mic của bạn.",
      hostUnmutedCam: "Host đã bật camera của bạn.",
      hostStoppedScreen: "Host đã dừng chia sẻ màn hình của bạn.",
      hostMutedAll: "Host đã tắt tiếng tất cả mọi người trong phòng.",
      allowSelfUnmute: "Cho phép học viên tự bật mic",
      allowSelfUnmuteRoom: "Cho phép thành viên tự bật mic",
      selfUnmuteOn: "Đã cho phép học viên tự bật mic.",
      selfUnmuteOnRoom: "Đã cho phép thành viên tự bật mic.",
      selfUnmuteOff: "Đã tắt quyền học viên tự bật mic.",
      selfUnmuteOffRoom: "Đã tắt quyền thành viên tự bật mic.",
      selfUnmuteBlocked: "Host đã tắt quyền tự bật mic. Vui lòng chờ host bật giùm.",
      allowSelfCamera: "Cho phép học viên tự bật camera",
      allowSelfCameraRoom: "Cho phép thành viên tự bật camera",
      selfCameraOn: "Đã cho phép học viên tự bật camera.",
      selfCameraOnRoom: "Đã cho phép thành viên tự bật camera.",
      selfCameraOff: "Đã tắt quyền học viên tự bật camera.",
      selfCameraOffRoom: "Đã tắt quyền thành viên tự bật camera.",
      selfCameraBlocked:
        "Host đã tắt quyền tự bật camera. Vui lòng chờ host bật giùm.",
      forbiddenMedia: "Bạn không có quyền điều khiển mic/camera.",
      forbiddenMuteAll: "Bạn không có quyền tắt toàn bộ mic.",
      forbiddenSelfUnmute: "Bạn không có quyền đổi chính sách này.",
      forbiddenSelfCamera: "Bạn không có quyền đổi chính sách này.",
      allowStudentShare: "Cho phép học viên chia sẻ màn hình",
      allowStudentShareRoom: "Cho phép thành viên chia sẻ màn hình",
      studentShareOn: "Đã cho phép học viên chia sẻ màn hình.",
      studentShareOnRoom: "Đã cho phép thành viên chia sẻ màn hình.",
      studentShareOff: "Đã tắt quyền học viên chia sẻ màn hình.",
      studentShareOffRoom: "Đã tắt quyền thành viên chia sẻ màn hình.",
      studentShareBlocked: "Host đã tắt quyền chia sẻ màn hình của học viên.",
      forbiddenStudentShare: "Bạn không có quyền đổi chính sách này.",
      allowMemberRecording: "Cho phép học viên ghi hình",
      allowMemberRecordingRoom: "Cho phép thành viên ghi hình",
      memberRecordingOn: "Đã cho phép học viên ghi hình.",
      memberRecordingOnRoom: "Đã cho phép thành viên ghi hình.",
      memberRecordingOff: "Đã tắt quyền học viên ghi hình.",
      memberRecordingOffRoom: "Đã tắt quyền thành viên ghi hình.",
      memberRecordingBlocked: "Host đã tắt quyền ghi hình đối với thành viên.",
      forbiddenRecord: "Bạn không có quyền quản lý ghi hình.",
      shareDeniedNoPerm: "Bạn không có quyền chia sẻ màn hình.",
      forbiddenStopScreen: "Bạn không có quyền dừng chia sẻ màn hình.",
      hostLoweredAllHands: "Host đã hạ tất cả các tay xuống.",
      hostAllowedRecording: "Host đã CHO PHÉP thành viên ghi hình cuộc họp.",
      hostDisabledRecording: "Host đã TẮT quyền ghi hình cuộc họp đối với thành viên.",
      hostAllowedPrivateAi: "Host đã CHO PHÉP thành viên sử dụng AI Chat riêng tư.",
      hostDisabledPrivateAi: "Host đã TẮT quyền sử dụng AI Chat riêng tư đối với thành viên.",
      kickedByHost: "Bạn đã bị Host mời ra khỏi phòng.",
      successMuteAll: "Đã tắt mic tất cả mọi người",
      cameraOffAll: "Tắt camera tất cả",
      cameraOffAllRoom: "Tắt camera tất cả thành viên",
      cameraOffAllDesc: "Tắt camera của tất cả học viên trong phòng.",
      cameraOffAllDescRoom: "Tắt camera của tất cả thành viên trong phòng.",
      confirmCameraOffAllTitle: "Tắt camera tất cả",
      confirmCameraOffAll:
        "Bạn có chắc muốn tắt camera của tất cả mọi người trong phòng?",
      successCameraOffAll: "Đã tắt camera tất cả mọi người (trừ bạn).",
      forbiddenCameraOffAll: "Bạn không có quyền tắt toàn bộ camera.",
      hostCameraOffAll: "Host đã tắt camera tất cả mọi người trong phòng.",
      successLowerAllHands: "Đã hạ {count} tay xuống",
      confirmLowerHandsTitle: "Hạ tất cả tay",
      confirmLowerHands:
        "Bạn có chắc muốn hạ tay của {count} người đang giơ tay?",
      noHandsRaised: "Không có ai đang giơ tay.",
      forbiddenLowerHands: "Bạn không có quyền hạ tay.",
      successStopScreen: "Đã dừng chia sẻ màn hình người dùng",
      successMuteMic: "Đã tắt mic người dùng",
      successMuteCam: "Đã tắt camera người dùng",
      successUnmuteMic: "Đã bật mic người dùng",
      successUnmuteCam: "Đã bật camera người dùng",
      successKick: "Đã mời người dùng ra khỏi phòng",
      successBan: "Đã mời người dùng ra khỏi phòng và cấm vào lại",
      bannedListTitle: "Danh sách bị cấm",
      bannedListDesc: "Danh sách các thành viên bị cấm tham gia lại phòng.",
      unban: "Gỡ cấm",
      noBannedMembers: "Chưa có thành viên nào bị cấm trong phòng này.",
      noBannedMembersTitle: "Chưa có thành viên nào bị cấm",
      noBannedMembersDesc: "Các thành viên bị Host mời ra khỏi phòng và cấm truy cập lại sẽ hiển thị tại đây.",
      successUnban: "Đã gỡ cấm thành viên thành công",
      bannedFromRoom: "Tài khoản của bạn đã bị cấm truy cập vào phòng này bởi Host.",
      forbiddenKick: "Bạn không có quyền mời thành viên ra khỏi phòng.",
      lockRoom: "Khóa phòng (chặn người mới)",
      lockOn: "Đã khóa phòng. Người mới không thể tham gia.",
      lockOff: "Đã mở khóa phòng.",
      forbiddenLock: "Bạn không có quyền khóa/mở phòng.",
      requireApproval: "Yêu cầu duyệt khi vào phòng",
      requireApprovalDesc: "Khi bật, người vào phải gõ cửa và chờ Host/Co-host duyệt trước khi vào.",
      requireApprovalOn: "Đã bật duyệt thủ công khi vào phòng.",
      requireApprovalOff: "Đã tắt duyệt thủ công. Mọi người vào tự do.",
      forbiddenRequireApproval: "Chỉ chủ phòng mới có quyền bật/tắt yêu cầu duyệt.",
      hostLockedRoom: "Host đã khóa phòng. Người mới không thể tham gia.",
      hostUnlockedRoom: "Host đã mở khóa phòng.",
      endLive: "Kết thúc buổi live",
      endLiveRoom: "Kết thúc phòng",
      confirmEndLiveTitle: "Kết thúc buổi live",
      confirmEndLiveTitleRoom: "Kết thúc phòng",
      confirmEndLive: "Kết thúc buổi live cho tất cả mọi người? Mọi người sẽ về màn hình kết thúc, tham gia lại sẽ tạo phiên mới. Lớp/phòng và điểm danh không đổi.",
      confirmEndLiveRoom: "Kết thúc phòng cho tất cả mọi người? Mọi người sẽ về màn hình kết thúc, tham gia lại sẽ tạo phiên mới. Phòng và điểm danh không đổi.",
      endLiveSuccess: "Đã kết thúc buổi live.",
      endLiveSuccessRoom: "Đã kết thúc phòng.",
      forbiddenEnd: "Bạn không có quyền kết thúc buổi live.",
      forbiddenEndRoom: "Bạn không có quyền kết thúc phòng.",
      hostEndedSession: "Host đã kết thúc buổi live.",
      hostRestrictedChat: "Bạn đã bị Host hạn chế gửi tin nhắn chat.",
      hostUnrestrictedChat: "Host đã gỡ hạn chế chat cho bạn.",
      hostRestrictedVoice: "Bạn đã bị Host hạn chế bật mic.",
      hostUnrestrictedVoice: "Host đã gỡ hạn chế mic cho bạn.",
      hostRestrictedVoiceAll: "Host đã hạn chế quyền bật mic của mọi người.",
      voiceRestrictedBlocked:
        "Bạn đang bị hạn chế bật mic. Vui lòng chờ Host gỡ hạn chế.",
      chatRestrictedBadge: "Hạn chế chat",
      voiceRestrictedBadge: "Hạn chế voice",
      hostBadge: "Chủ phòng",
      coHostBadge: "Co-host",
      emptyRoomTitle: "Chỉ có bạn trong phòng",
      emptyRoomHint: "Mời thành viên tham gia để bắt đầu trò chuyện.",
    },
    waitingQueue: {
      tab: "Chờ",
      loading: "Đang tải danh sách chờ...",
      empty: "Chưa có ai đang chờ duyệt.",
      userLabel: "Người dùng",
      pendingLabel: "Đang chờ duyệt",
      rejectedLabel: "Đã từ chối trước đó",
      enrollmentPending: "Chờ duyệt ghi danh",
      admit: "Duyệt",
      reject: "Từ chối",
      invite: "Mời",
      admitSuccess: "Đã duyệt vào phòng.",
      rejectSuccess: "Đã từ chối yêu cầu vào phòng.",
      inviteSuccess: "Đã gửi lời mời vào phòng.",
      inviteFailed: "Không thể gửi lời mời. Vui lòng thử lại.",
      forbiddenAdmit: "Bạn không có quyền duyệt phòng chờ.",
      knock: "Gõ cửa",
      knockHint: "Phòng bật chế độ duyệt? Gõ cửa để xin vào.",
      knockTitle: "Xin vào phòng",
      knockDesc: "Phòng đang bật chế độ duyệt. Hãy gõ cửa và chờ host/co-host cho vào.",
      knockSuccess: "Đã gửi yêu cầu vào phòng. Vui lòng chờ duyệt.",
      knockFailed: "Không thể gửi yêu cầu. Vui lòng thử lại.",
      waitingTitle: "Đang chờ duyệt vào phòng...",
      waitingDesc: "Yêu cầu của bạn đã được gửi. Màn hình sẽ tự động mở khi host/co-host duyệt.",
      rejectedTitle: "Yêu cầu vào phòng bị từ chối",
      rejectedHint: "Bạn đã bị từ chối. Vui lòng chờ được mời lại mới có thể xin vào tiếp.",
      admittedTitle: "Đã được duyệt vào phòng!",
      admittedToast: "Đã được duyệt vào phòng!",
      lockedHint: "Phòng đã bị khóa. Người mới không thể tham gia lúc này.",
      cancel: "Hủy yêu cầu",
      cancelSuccess: "Đã hủy yêu cầu vào phòng.",
      cancelledHint: "Yêu cầu đã hủy. Bạn có thể gõ cửa lại.",
      back: "Quay lại",
      waitingElapsed: "Đang chờ {time}",
    },
    webviewBlock: {
      title: "Mở trong trình duyệt",
      genericApp: "Ứng dụng này",
      description:
        "Bạn đang dùng trình duyệt tích hợp của {app}, chưa được hỗ trợ đầy đủ.",
      instruction: "Cách mở:",
      copyLink: "Sao chép liên kết",
      copied: "Đã sao chép!",
      openInChrome: "Mở bằng Chrome",
      step1: "Sao chép liên kết ở trên",
      step2: "Mở Chrome hoặc Safari",
      step3: "Dán liên kết vào trình duyệt của bạn",
    },
    errorBoundary: {
      title: "Cuộc gọi video gặp lỗi",
      defaultDescription:
        "Đã xảy ra lỗi không mong muốn trong phiên gọi video.",
      reloadPage: "Tải lại trang",
      copyLog: "Sao chép nhật ký",
      copied: "Đã sao chép!",
      goHome: "Về trang chủ",
      technicalDetails: "Chi tiết kỹ thuật",
      stackTrace: "Dấu vết ngăn xếp:",
      componentStack: "Ngăn xếp thành phần:",
    },
    inviteParticipant: "Mời tham gia phòng",
    sendInvite: "Gửi lời mời",
    inviteSubtitle: "Gửi lời mời tham gia đến bạn bè của bạn",
    inviteDescription:
      "Chọn bạn bè bạn muốn mời vào phòng này. Họ sẽ nhận được thông báo kèm liên kết để tham gia.",
    inviteSelectedTitle: "Đã chọn",
    inviteEmptyHint: "Chọn ít nhất một người bạn để gửi lời mời.",
    inviteReadyHint: "Sẵn sàng gửi lời mời đến {{count}} người bạn.",
    inviteRemove: "Bỏ chọn {{name}}",
  },
  notifications: {
    inviteSent: "Đã gửi lời mời",
  },
  roomFullModal: {
    title: "Phòng đã đầy",
    message: "Phòng này đã đạt số lượng người tham gia tối đa.",
    subMessage: "Vui lòng chọn phòng khác hoặc thử lại sau.",
    close: "Đóng",
  },
  passwordScreen: {
    title: "Phòng riêng tư",
    description: "Phòng này là phòng riêng tư. Nhập mật khẩu để tham gia.",
    label: "Mật khẩu",
    placeholder: "Nhập mật khẩu phòng",
    submit: "Tham gia",
    verifying: "Đang xác minh...",
    incorrectPassword: "Mật khẩu không đúng. Vui lòng thử lại.",
    roomNotFound: "Không tìm thấy phòng.",
    genericError: "Đã xảy ra lỗi. Vui lòng thử lại.",
  },
  beauty: {
    title: "Làm đẹp",
    tabLabel: "Làm đẹp",
    smoothing: "Làm mịn da",
    brightness: "Tăng độ sáng",
    warmth: "Tông màu ấm",
    colorFilter: "Bộ lọc màu (Sống động)",
    eyeBrighten: "Sáng mắt",
    teethWhiten: "Trắng răng",
    faceSlim: "Thon mặt",
    eyeEnlarge: "Mắt to",
    faceAwareHint: "Nhận diện khuôn mặt",
    statusActive: "Đang hoạt động",
    statusUnsupported: "Không hỗ trợ",
    statusInitializing: "Đang khởi tạo...",
    statusError: "Lỗi",
    attached: "Đã kích hoạt hiệu ứng làm đẹp",
    unsupported:
      "Không hỗ trợ hiệu ứng làm đẹp — thiết bị của bạn có thể không hỗ trợ xử lý video. Hãy thử dùng Chrome hoặc Edge trên máy tính.",
    attachFailed:
      "Không hỗ trợ hiệu ứng làm đẹp — thiết bị của bạn có thể không hỗ trợ xử lý video.",
    trackTimeout: "Không thể gắn hiệu ứng làm đẹp — không tìm thấy camera.",
  },
  virtualBackground: {
    title: "Phông nền",
    none: "Không có",
    upload: "Tải lên",
    fileTooLarge: "Tệp quá lớn. Kích thước tối đa là 3MB.",
    uploadSuccess: "Đã tải lên và áp dụng phông nền",
    uploadFailed: "Không thể tải lên phông nền",
    setFailed: "Không thể cài đặt phông nền ảo",
    loading: "Đang tải...",
  },
  topicBackground: {
    title: "Chủ đề",
    none: "Không có",
    upload: "Tải lên",
    loading: "Đang tải...",
  },
  avatarPicker: {
    title: "Ảnh đại diện cuộc họp",
    tabLabel: "Ảnh đại diện",
    success: "Cập nhật ảnh đại diện thành công",
    error: "Cập nhật ảnh đại diện thất bại",
    description:
      "Tải lên hình ảnh để đổi ảnh đại diện phòng họp. Nếu để trống, hệ thống sẽ dùng ảnh đại diện chính.",
    uploadImage: "Tải lên ảnh mới",
    invalidFile: "Vui lòng chọn tệp hình ảnh hợp lệ",
    cropTitle: "Cắt ảnh đại diện phòng họp",
    resetToProfile: "Dùng ảnh đại diện chính",
    resetSuccess: "Đã dùng ảnh đại diện chính",
    saving: "Đang lưu...",
    saveAvatar: "Lưu ảnh đại diện",
  },
  game: {
    crackIt: {
      title: "Crack It",
      close: "Đóng",
      waitingStart: "Đang chuẩn bị ván đấu...",
      roundResult: "Đáp án chính xác là",
      nextRoundIn: "Ván tiếp theo sẽ bắt đầu sau ít giây...",
      gameOver: "KẾT THÚC!",
      finalLeaderboard: "Bảng Xếp Hạng Chung Cuộc",
      closeGame: "Đóng Game",
      round: "Ván",
      hint: "Gợi ý",
      answerLength: "Số từ",
      youGotIt: "Bạn đã đoán đúng! Đợi người khác...",
      typeAnswer: "Nhập đáp án của bạn...",
      gotItRight: "đã đoán đúng!",
      leaderboard: "Bảng xếp hạng",
      waitingPlayers: "Đang đợi người chơi...",
      playGame: "Chơi Crack It",
      notEnoughPlayers: "Cần ít nhất 2 người để bắt đầu trò chơi!",
      you: "Bạn",
      playerX: "Người chơi {0}",
      exitGameTooltip: "Thoát trò chơi",
      exitConfirmTitle: "Xác nhận thoát",
      exitConfirmDesc1: "Bạn có chắc chắn muốn thoát khỏi trò chơi này không?",
      exitConfirmDesc2: "Bạn sẽ không thể tham gia lại ván chơi này nữa!",
      cancel: "Hủy",
      confirmExit: "Đồng ý thoát",
      roundLeaderboard: "Thành tích ván này",
      forceStopNotEnoughPlayers:
        "Không đủ người chơi tiếp tục. Trò chơi đã bị hủy.",
      forceStopGeneric: "Trò chơi bị dừng đột ngột.",
      out: "Thoát",
      tallyingFinalResults: "Đang tổng hợp kết quả chung cuộc...",
      nextRoundIn: "Ván tiếp theo sẽ bắt đầu sau ít giây...",
      gameHistory: "Lịch sử trò chơi",
      historyTitle: "Lịch sử thi đấu",
      noHistoryFound: "Chưa có dữ liệu lịch sử cho phòng này.",
      score: "Điểm",
      matchDate: "Ngày chơi",
      winner: "Người chiến thắng",
    },
    setup: {
      title: "Thiết lập Trò chơi",
      pictureItDesc:
        "Miêu tả hình ảnh bằng ngôn ngữ của phòng. Những người chơi khác sẽ đánh giá mô tả của bạn.",
      crackItDesc: "Đoán từ khóa ẩn qua các gợi ý trước những người chơi khác.",
      difficultyDesc: "Điều chỉnh độ khó phù hợp cho phòng của bạn.",
      languageDesc: "Chọn ngôn ngữ trò chơi cho phiên này.",
      selectLanguage: "Ngôn ngữ",
      langEn: "Tiếng Anh",
      langZh: "Tiếng Trung",
      selectLevel: "Mức độ",
      levelEasy: "Dễ",
      levelMedium: "Trung bình",
      levelHard: "Khó",
      cancel: "Hủy",
      startNow: "Bắt đầu ngay",
    },
    pictureIt: {
      topBar: {
        round: "Vòng",
        describer: "Người mô tả",
        spectator: "Khán giả",
        roleDescriber: "Người mô tả",
        roleRater: "Người đoán",
      },
      actionPanel: {
        watchingAsSpectator: "Bạn đang xem với tư cách khán giả.",
        turnOnMic: "Bật Mic (Bắt đầu)",
        turnOffMic: "Tắt Mic (Kết thúc)",
        ratersAreScoring: "Đang chờ chấm điểm... ({0}s)",
        yourRating: "Điểm của bạn:",
        timeLeft: "(Còn {0}s)",
        submitRating: "Gửi điểm",
        ratingSubmitted: "Đã gửi! Đang chờ người khác...",
        waiting: "Đang chờ...",
      },
      imageCard: {
        describeToTeam: "Hãy mô tả bức ảnh này cho đội của bạn",
        listenAndRate: "Hãy lắng nghe và chuẩn bị chấm điểm",
        loadingImage: "Đang tải ảnh...",
        imageUnavailable: "Không tải được ảnh",
        describerSpeaking: "Người mô tả đang nói...",
        preparing: "Đang chuẩn bị...",
        forbiddenWordsTitle: "Từ cấm",
        forbiddenWordsDesc: "KHÔNG được sử dụng các từ này",
        flaggedCount: "{0} / {1} người đã báo vi phạm",
      },
      modals: {
        gameEnded: "Kết thúc trò chơi",
        notEnoughPlayers: "Không đủ người chơi để tiếp tục.",
        close: "Đóng",
        waitingStart: "Đang chuẩn bị ván đấu...",
      },
      badges: {
        "Master Describer": "Bậc thầy mô tả",
        "Great Speaker": "Diễn giả xuất sắc",
        "Best Rating Accuracy": "Đánh giá chuẩn xác",
        "Most Fluent": "Trôi chảy nhất",
        "Team Player": "Tinh thần đồng đội",
        "Keep Practicing": "Cố gắng hơn nhé",
        champion: "Nhà vô địch",
      },
      gameOver: {
        title: "Trò chơi kết thúc",
        thanksForPlaying: "Cảm ơn bạn đã chơi",
        finalLeaderboard: "Bảng xếp hạng chung cuộc",
        roundsPlayed: "Số vòng đã chơi",
        language: "Ngôn ngữ",
        difficulty: "Độ khó",
        winningScore: "Điểm chiến thắng",
        highestRoundScore: "Điểm 1 vòng cao nhất",
        playAgain: "Chơi lại",
        backToRoom: "Về phòng",
        playersCount: "{0} người chơi",
      },
      leaderboard: {
        title: "Bảng xếp hạng",
        playersCount: "{0} người chơi",
      },
      roundResult: {
        nextRoundStartsIn: "Vòng tiếp theo bắt đầu sau...",
        averageRating: "Điểm trung bình",
      },
    },
  },
  participants: "người",
  noLimit: "Không giới hạn",
  copySuccess: "Đã sao chép liên kết",
  copyLinkTooltip: "Sao chép liên kết",
  expiredBadge: "Đã hết hạn",
  roomExpiredToast: "Phòng này đã hết thời hạn sử dụng!",
  callEnded: {
    titleExpired: "Phiên này đã hết hạn",
    titleLeft: "Bạn đã rời khỏi cuộc họp",
    titleEnded: "Buổi live đã kết thúc",
    descExpired:
      "Đã đạt đến giới hạn thời gian của phòng và phiên đã tự động đóng.",
    descLeft:
      "Bạn có thể đóng cửa sổ này một cách an toàn hoặc quay lại màn hình chính.",
    descEnded:
      "Host đã kết thúc buổi live cho tất cả mọi người. Bạn có thể tham gia lại để bắt đầu phiên mới.",
    rejoin: "Tham gia lại",
    returnHome: "Quay lại trang chủ",
    expiredToast: "Cuộc gọi đã kết thúc do hết thời lượng phòng",
    endedToast: "Buổi live đã kết thúc",
  },
  breakoutRooms: {
    mainRoom: "Phòng chính",
    youAreHere: "Bạn ở đây",
    emptyRoom: "Phòng trống",
    joinRoomBtn: "Tham gia phòng",
    allAssigned: "Tất cả học viên đã được phân phối",
    noActiveStudents: "Không có học viên nào hoạt động trong phòng học.",
    randomSuccess: "Phân chia ngẫu nhiên thành công!",
    assignAtLeastOne: "Vui lòng phân bổ ít nhất một học viên vào phòng nhỏ.",
    saveAndStartSuccess: "Đã lưu cấu hình và bắt đầu phòng nhỏ thành công!",
    saveSuccess: "Lưu cấu hình chia phòng thành công!",
    saveError: "Lỗi lưu cấu hình chia phòng.",
    setupTitle: "Thiết lập Breakout Rooms",
    roomCount: "Số lượng phòng",
    breakoutRoomOption: "Phòng Nhỏ",
    configTitle: "Cấu hình phòng nhỏ",
    timer: "Hẹn giờ thảo luận",
    minutes: "phút",
    timerMinutes: "Hẹn giờ (phút)",
    allowChangeGroup: "Cho phép đổi nhóm",
    allowChangeRoom: "Cho phép đổi phòng",
    memberLimit: "Giới hạn thành viên",
    membersPerRoom: "thành viên/phòng",
    randomAllocationBtn: "Phân bổ ngẫu nhiên",
    noStudentsYet: "Chưa có học viên nào tham gia phòng học.",
    studentsCount: "học viên",
    dragOrRandom: "Kéo học viên vào đây hoặc phân ngẫu nhiên",
    moveToRoom: "Chuyển: Phòng ",
    cancelBtn: "Hủy",
    saveConfigBtn: "Lưu cấu hình",
    startBtn: "Bắt đầu nhóm nhỏ",
    broadcastBtn: "Gửi thông báo chung",
    broadcastTitle: "Gửi thông báo chung",
    closeRoomsBtn: "Đóng phòng",
    broadcastPlaceholder: "Ví dụ: Các em thảo luận trong 5 phút nữa nhé...",
    broadcastDesc:
      "Tin nhắn này sẽ được gửi đồng thời đến tất cả các phòng thảo luận nhỏ.",
    sendBtn: "Gửi",
    closeError: "Lỗi đóng các phòng nhỏ.",
    moveError: "Lỗi di chuyển học viên.",
    joinError: "Lỗi tham gia phòng nhỏ.",
    returnMainError: "Lỗi quay trở lại phòng chính.",
    broadcastError: "Lỗi gửi thông báo.",
    broadcastToastTitle: "Thông báo từ Host",
    toggleAllowError: "Không thể thay đổi cài đặt chuyển phòng.",
    breakoutRoomPrefix: "Phòng thảo luận: ",
    freeToChange: "Tự do chuyển",
    fixedRoom: "Phòng cố định",
    shuffleBtn: "Trộn",
    resetBtn: "Đặt lại",
    startError: "Lỗi khởi chạy phòng nhỏ.",
    openRoomsBtn: "Mở phòng",
    moveTo: "Di chuyển sang",
    moveToMain: "Di chuyển về phòng chính",
    isFull: "(Đã đầy)",
    host: "Host",
    userPrefix: "User #",
    breakoutRoomDefaultName: "Phòng thảo luận",
    switchError: "Không thể chuyển phòng",
    notStartedTitle: "Chưa bắt đầu chia nhóm",
    notStartedDesc:
      "Giáo viên chưa mở phòng thảo luận nhóm nhỏ cho buổi học này.",
    lowSpeakingNotice: "⚠️ Có {count} học viên chưa đạt ngưỡng phát biểu",
    lowSpeakingBadge: "⚠️ {count} HV chưa đạt",
    changeLocked: "Đã khóa chuyển phòng",
    roomFull: "Phòng đã đầy",
    roomPrefix: "Phòng ",
    returningTitle: "Quay lại phòng chính",
    movingTitle: "Di chuyển vào phòng nhóm",
    returningDesc:
      "Cuộc thảo luận nhóm đã kết thúc. Bạn đang được tự động chuyển về phòng học chính...",
    movingDesc: "Bạn đang được tự động chuyển sang phòng thảo luận nhóm...",
    roomFullError: "Không thể di chuyển vì phòng đã đầy.",
    setupModeTip: "Chế độ phân bổ phòng: Xếp nhóm học viên rồi nhấn nút \"Mở phòng\" bên dưới để chính thức đưa các thành viên vào phòng.",
    setupStageHint: "Đã xếp học viên vào phòng. Nhấn 'Mở phòng' bên dưới để áp dụng.",
  },
  game: {
    crackIt: {
      title: "Crack It",
      close: "Đóng",
      waitingStart: "Đang chuẩn bị ván đấu...",
      roundResult: "Đáp án chính xác là",
      nextRoundIn: "Ván tiếp theo sẽ bắt đầu sau ít giây...",
      gameOver: "KẾT THÚC!",
      finalLeaderboard: "Bảng Xếp Hạng Chung Cuộc",
      closeGame: "Đóng Game",
      round: "Ván",
      hint: "Gợi ý",
      answerLength: "Số từ",
      youGotIt: "Bạn đã đoán đúng! Đợi người khác...",
      typeAnswer: "Nhập đáp án của bạn...",
      gotItRight: "đã đoán đúng!",
      leaderboard: "Bảng xếp hạng",
      waitingPlayers: "Đang đợi người chơi...",
      playGame: "Chơi Crack It",
      notEnoughPlayers: "Cần ít nhất 2 người để bắt đầu trò chơi!",
      you: "Bạn",
      playerX: "Người chơi {0}",
      exitGameTooltip: "Thoát trò chơi",
      exitConfirmTitle: "Xác nhận thoát",
      exitConfirmDesc1: "Bạn có chắc chắn muốn thoát khỏi trò chơi này không?",
      exitConfirmDesc2: "Bạn sẽ không thể tham gia lại ván chơi này nữa!",
      cancel: "Hủy",
      confirmExit: "Đồng ý thoát",
      roundLeaderboard: "Thành tích ván này",
      forceStopNotEnoughPlayers:
        "Không đủ người chơi tiếp tục. Trò chơi đã bị hủy.",
      forceStopGeneric: "Trò chơi bị dừng đột ngột.",
      out: "Thoát",
      tallyingFinalResults: "Đang tổng hợp kết quả chung cuộc...",
      nextRoundIn: "Ván tiếp theo sẽ bắt đầu sau ít giây...",
      gameHistory: "Lịch sử trò chơi",
      historyTitle: "Lịch sử thi đấu",
      noHistoryFound: "Chưa có dữ liệu lịch sử cho phòng này.",
      score: "Điểm",
      matchDate: "Ngày chơi",
      winner: "Người chiến thắng",
      startDateLabel: "Từ ngày",
      endDateLabel: "Đến ngày",
    },
    setup: {
      title: "Thiết lập Trò chơi",
      pictureItDesc:
        "Miêu tả hình ảnh bằng ngôn ngữ của phòng. Những người chơi khác sẽ đánh giá mô tả của bạn.",
      crackItDesc: "Đoán từ khóa ẩn qua các gợi ý trước những người chơi khác.",
      difficultyDesc: "Điều chỉnh độ khó phù hợp cho phòng của bạn.",
      languageDesc: "Chọn ngôn ngữ trò chơi cho phiên này.",
      selectLanguage: "Ngôn ngữ",
      langEn: "Tiếng Anh",
      langZh: "Tiếng Trung",
      langVi: "Tiếng Việt",
      languageLockedByCommunity:
        "Ngôn ngữ trò chơi ăn theo ngôn ngữ của cộng đồng.",
      lockedHint: "Đã khóa",
      selectLevel: "Mức độ",
      levelEasy: "Dễ",
      levelMedium: "Trung bình",
      levelHard: "Khó",
      cancel: "Hủy",
      startNow: "Bắt đầu ngay",
    },
    pictureIt: {
      topBar: {
        round: "Vòng",
        describer: "Người mô tả",
        spectator: "Khán giả",
        roleDescriber: "Người mô tả",
        roleRater: "Người đoán",
      },
      actionPanel: {
        watchingAsSpectator: "Bạn đang xem với tư cách khán giả.",
        turnOnMic: "Bật Mic (Bắt đầu)",
        turnOffMic: "Tắt Mic (Kết thúc)",
        ratersAreScoring: "Đang chờ chấm điểm... ({0}s)",
        yourRating: "Điểm của bạn:",
        timeLeft: "(Còn {0}s)",
        submitRating: "Gửi điểm",
        ratingSubmitted: "Đã gửi! Đang chờ người khác...",
        waiting: "Đang chờ...",
      },
      imageCard: {
        describeToTeam: "Hãy mô tả bức ảnh này cho đội của bạn",
        listenAndRate: "Hãy lắng nghe và chuẩn bị chấm điểm",
        loadingImage: "Đang tải ảnh...",
        imageUnavailable: "Không tải được ảnh",
        describerSpeaking: "Người mô tả đang nói...",
        preparing: "Đang chuẩn bị...",
        forbiddenWordsTitle: "Từ cấm",
        forbiddenWordsDesc: "KHÔNG được sử dụng các từ này",
        flaggedCount: "{0} / {1} người đã báo vi phạm",
      },
      modals: {
        gameEnded: "Kết thúc trò chơi",
        notEnoughPlayers: "Không đủ người chơi để tiếp tục.",
        close: "Đóng",
        waitingStart: "Đang chuẩn bị ván đấu...",
      },
      badges: {
        "Master Describer": "Bậc thầy mô tả",
        "Great Speaker": "Diễn giả xuất sắc",
        "Best Rating Accuracy": "Đánh giá chuẩn xác",
        "Most Fluent": "Trôi chảy nhất",
        "Team Player": "Tinh thần đồng đội",
        "Keep Practicing": "Cố gắng hơn nhé",
        champion: "Nhà vô địch",
      },
      gameOver: {
        title: "Trò chơi kết thúc",
        thanksForPlaying: "Cảm ơn bạn đã chơi",
        finalLeaderboard: "Bảng xếp hạng chung cuộc",
        roundsPlayed: "Số vòng đã chơi",
        language: "Ngôn ngữ",
        difficulty: "Độ khó",
        winningScore: "Điểm chiến thắng",
        highestRoundScore: "Điểm 1 vòng cao nhất",
        playAgain: "Chơi lại",
        backToRoom: "Về phòng",
        playersCount: "{0} người chơi",
      },
      leaderboard: {
        title: "Bảng xếp hạng",
        playersCount: "{0} người chơi",
      },
      roundResult: {
        nextRoundStartsIn: "Vòng tiếp theo bắt đầu sau...",
        averageRating: "Điểm trung bình",
      },
    },
  },

  coHost: {
    manage: "Quản lý co-host",
    add: "Thêm Co-host",
    manageShort: "Quản lý",
    emptyShort: "Chưa phân công Co-host",
    permissionsUpdated: "Đã cập nhật quyền co-host.",
    replaced: "Đã thay thế co-host.",
    assigned: "Đã phân công co-host.",
    assignError: "Không thể phân công co-host. Vui lòng thử lại.",
    revoked: "Đã gỡ phân công co-host.",
    revokeError: "Không thể gỡ co-host. Vui lòng thử lại.",
    remove: "Gỡ",
    assignCoHost: "Phân công Co-host",
    saveChanges: "Lưu thay đổi",
    confirmRevokeTitle: "Xác nhận gỡ co-host",
    confirmRevokeMessage: "Bạn có chắc muốn gỡ phân công co-host này? Hành động này không thể hoàn tác.",
    confirmClearMessage: "Bỏ người dùng đang chọn khỏi biểu mẫu?",
    clearSelection: "Bỏ chọn",
    emptyClass: "Chưa có Co-host được phân công cho lớp học",
    emptyRoom: "Chưa có Co-host được phân công cho phòng này",
    revokeSuccessTitle: "Đã gỡ phân công Co-host!",
    revokeSuccessMessage: "Phân công Co-host đã được gỡ thành công.",
    done: "Hoàn tất",
    cancel: "Hủy",
    delete: "Xóa",
    saving: "Đang lưu...",
    step1: "1. Chọn người dùng",
    userLabel: "Người dùng *",
    searchPlaceholder: "Tìm kiếm theo tên hoặc email",
    noUserFound: "Không tìm thấy người dùng phù hợp.",
    step2: "2. Chọn quyền cho Co-host",
    quickSelect: "Chọn nhanh",
    selectAll: "Chọn tất cả",
    deselectAll: "Bỏ chọn tất cả",
    selectPermissions: "Chọn các quyền mà Co-host sẽ có trong lớp học này",
    note: "Lưu ý:",
    noteDesc: "Quyền của co-host chỉ áp dụng cho mỗi lớp học.",
    noteDescRoom: "Quyền của co-host chỉ áp dụng cho mỗi phòng.",
    scopeClass: 'trong lớp học "{name}"',
    scopeRoom: 'trong phòng "{name}"',
    scopeTypeRoom: "Phòng",
    scopeTypeClass: "Lớp học",
    headerSubtitle: "Chỉ định trợ giảng và quản lý quyền hạn điều hành phòng",
    headerSubtitleRoom:
      "Chỉ định người hỗ trợ và quản lý quyền hạn điều hành phòng",
    userSectionTitle: "Người dùng đảm nhiệm",
    permissionSectionTitle: "Thiết lập quyền hạn",
    permissionsCount: "{count}/{total} quyền",
    changeUser: "Đổi người khác",
    designatedBadge: "Co-host chỉ định",
    inRoomStatus: "Đang trong phòng",
    removeUser: "Gỡ người dùng này",
    noCandidatesTitle: "Chưa có thành viên khả dụng trong phòng",
    noCandidatesDesc: "Chỉ những thành viên đang trực tuyến trong phòng mới có thể được chỉ định làm Co-host.",
    noCandidatesRoomTitle: "Chưa có ai để chọn",
    noCandidatesRoomDesc:
      "Chưa có bạn bè hoặc thành viên nào để chọn. Hãy kết bạn hoặc mời họ vào phòng trước.",
    inRoomBadge: "Trong phòng",
    absentStatus: "Không ở trong phòng",
    notInRoomHint:
      "Người này không ở trong phòng. Quyền Co-host sẽ áp dụng khi họ vào phòng.",
    quickPresetsLabel: "Gợi ý chọn nhanh:",
    presetAssistant: "Gói Trợ giảng",
    presetAssistantRoom: "Gói Hỗ trợ điều hành",
    selectGroup: "Chọn nhóm",
    deselectGroup: "Bỏ nhóm",
    scopeDescriptionRoom: "Chọn các quyền mà Co-host sẽ có trong phòng này",
    scopeDescriptionClass: "Chọn các quyền mà Co-host sẽ có trong lớp học này",
    groupRoomSecurity: "Bảo mật phòng",
    groupStudentManagementRoom: "Quản lý thành viên",
    groupMemberModerationRoom: "Kiểm duyệt thành viên",
    badgeSensitive: "Nhạy cảm",
    badgeCaution: "Chú ý",
    hintNoUser: "Vui lòng chọn người dùng được phân công Co-host.",
    hintNoPerms: "Vui lòng chọn ít nhất 1 quyền hạn cho Co-host.",
    hintNoChange: "Chưa có thay đổi nào so với quyền hạn hiện tại.",
    scopeNoticeRoom: 'Quyền của Co-host chỉ có hiệu lực trong phòng "{name}".',
    scopeNoticeClass: 'Quyền của Co-host chỉ có hiệu lực trong lớp học "{name}".',
    permissionHelpers: {
      mic_toggle: "Cho phép hoặc giới hạn mic của từng thành viên",
      camera_toggle: "Bật hoặc tắt camera của thành viên",
      mute_all: "Tắt mic tất cả học viên trong phòng",
      allow_self_unmute: "Cho phép học viên tự mở lại mic",
      allow_self_camera: "Cho phép học viên tự bật lại camera",
      remove_student: "Mời học viên rời khỏi phòng học",
      admit_waiting: "Duyệt học viên từ phòng chờ vào lớp",
      camera_off_all: "Tắt camera của toàn bộ học viên đang bật",
      block_all_mics: "Tắt mic toàn bộ và không cho học viên tự bật lại",
      lower_all_hands: "Hạ tay của toàn bộ học viên đang giơ tay",
      restrict_chat: "Không cho học viên gửi tin nhắn trong phòng",
      restrict_voice: "Tắt mic của học viên và không cho tự bật lại",
      stop_member_share:
        "Dừng phần chia sẻ màn hình đang diễn ra của học viên",
      lock_class: "Khóa phòng, không cho thêm người vào",
      end_class: "Đóng phòng và kết thúc phiên học",
      share_screen: "Chia sẻ màn hình hoặc cửa sổ bài giảng",
      manage_student_share: "Bật/tắt quyền cho học viên chia sẻ màn hình",
      allow_member_recording: "Bật/tắt quyền cho học viên tự ghi hình",
      record: "Bắt đầu hoặc dừng ghi hình phiên học",
    },
    permissionHelpersRoom: {
      mic_toggle: "Cho phép hoặc giới hạn mic của từng thành viên",
      camera_toggle: "Bật hoặc tắt camera của thành viên",
      mute_all: "Tắt mic tất cả thành viên trong phòng",
      allow_self_unmute: "Cho phép thành viên tự mở lại mic",
      allow_self_camera: "Cho phép thành viên tự bật lại camera",
      remove_student: "Mời thành viên rời khỏi phòng",
      admit_waiting: "Duyệt thành viên từ phòng chờ vào phòng",
      camera_off_all: "Tắt camera của toàn bộ thành viên đang bật",
      block_all_mics: "Tắt mic toàn bộ và không cho thành viên tự bật lại",
      lower_all_hands: "Hạ tay của toàn bộ thành viên đang giơ tay",
      restrict_chat: "Không cho thành viên gửi tin nhắn trong phòng",
      restrict_voice: "Tắt mic của thành viên và không cho tự bật lại",
      stop_member_share:
        "Dừng phần chia sẻ màn hình đang diễn ra của thành viên",
      lock_class: "Khóa phòng, không cho thêm người vào",
      end_class: "Đóng phòng và kết thúc phiên hoạt động",
      share_screen: "Chia sẻ màn hình hoặc cửa sổ trong phòng",
      manage_student_share: "Bật/tắt quyền cho thành viên chia sẻ màn hình",
      allow_member_recording: "Bật/tắt quyền cho thành viên tự ghi hình",
      record: "Bắt đầu hoặc dừng ghi hình trong phòng",
    },
    groups: {
      student_management: "Quản lý học viên",
      member_moderation: "Kiểm duyệt học viên",
      room_security: "Bảo mật lớp học",
    },
    permissions: {
      mic_toggle: "Bật/tắt quyền sử dụng mic",
      camera_toggle: "Bật/tắt quyền sử dụng camera",
      mute_all: "Tắt toàn bộ mic của học viên",
      allow_self_unmute: "Cho phép học viên tự bật mic",
      allow_self_camera: "Cho phép học viên tự bật camera",
      remove_student: "Xóa học viên khỏi lớp học",
      admit_waiting: "Duyệt học viên từ phòng chờ",
      camera_off_all: "Tắt toàn bộ camera của học viên",
      block_all_mics: "Chặn mic toàn bộ học viên",
      lower_all_hands: "Hạ tay toàn bộ học viên",
      restrict_chat: "Hạn chế chat của học viên",
      restrict_voice: "Hạn chế voice của học viên",
      stop_member_share: "Dừng chia sẻ màn hình của học viên",
      lock_class: "Khóa lớp học",
      end_class: "Kết thúc lớp học",
      share_screen: "Chia sẻ màn hình/cửa sổ",
      manage_student_share:
        "Quản lý quyền chia sẻ màn hình/cửa sổ của học viên",
      allow_member_recording: "Cho phép học viên ghi hình",
      record: "Bắt đầu/dừng ghi hình",
    },
    permissionsRoom: {
      mic_toggle: "Bật/tắt quyền sử dụng mic",
      camera_toggle: "Bật/tắt quyền sử dụng camera",
      mute_all: "Tắt toàn bộ mic của thành viên",
      allow_self_unmute: "Cho phép thành viên tự bật mic",
      allow_self_camera: "Cho phép thành viên tự bật camera",
      remove_student: "Xóa thành viên khỏi phòng",
      admit_waiting: "Duyệt thành viên từ phòng chờ",
      camera_off_all: "Tắt toàn bộ camera của thành viên",
      block_all_mics: "Chặn mic toàn bộ thành viên",
      lower_all_hands: "Hạ tay toàn bộ thành viên",
      restrict_chat: "Hạn chế chat của thành viên",
      restrict_voice: "Hạn chế voice của thành viên",
      stop_member_share: "Dừng chia sẻ màn hình của thành viên",
      lock_class: "Khóa phòng",
      end_class: "Kết thúc phòng",
      share_screen: "Chia sẻ màn hình/cửa sổ",
      manage_student_share:
        "Quản lý quyền chia sẻ màn hình/cửa sổ của thành viên",
      allow_member_recording: "Cho phép thành viên ghi hình",
      record: "Bắt đầu/dừng ghi hình",
    },
    errors: {
      COHOST_PERMISSION_REQUIRED: "Phải chọn ít nhất một quyền cho co-host.",
      COHOST_PERMISSION_INVALID: "Quyền co-host không hợp lệ.",
      COHOST_ROOM_TYPE_UNSUPPORTED: "Chỉ phòng Lớp học và phòng Pro/Custom mới hỗ trợ co-host.",
      COHOST_NOT_HOST: "Chỉ chủ phòng mới được phân công co-host.",
      COHOST_CANNOT_ASSIGN_OWNER: "Không thể phân công chủ phòng/lớp làm co-host.",
      COHOST_TARGET_NOT_ONLINE: "Chỉ người đang online trong phòng mới được làm co-host.",
      COHOST_TARGET_BANNED: "Người dùng đang bị cấm không thể làm co-host.",
      COHOST_TARGET_NOT_CONFIRMED: "Chỉ học viên đã xác nhận (Confirmed) mới được làm co-host.",
      COHOST_TARGET_NOT_CONFIRMED_ROOM:
        "Chỉ thành viên đã xác nhận (Confirmed) mới được làm co-host.",
      COHOST_NO_ASSIGNMENT: "Chưa có co-host được phân công.",
      COHOST_FORBIDDEN: "Co-host không có quyền thực hiện thao tác này.",
      ROOM_NOT_CREATED: "Lớp học chưa có phòng. Vui lòng thử lại sau khi phòng được tạo.",
    },
  },
  notifications: {
    inviteSent: "Đã gửi lời mời",
    room_invite: {
      title: "Lời mời tham gia phòng",
      body: "{inviterName} đã mời bạn tham gia phòng họp",
    },
    co_host_assigned: {
      title: "Được phân công làm Co-host",
      body: "{hostName} đã phân công bạn làm co-host {scope}",
    },
    co_host_updated: {
      title: "Quyền Co-host được cập nhật",
      body: "{hostName} đã cập nhật quyền co-host của bạn {scope}",
    },
    co_host_revoked: {
      title: "Đã gỡ phân công Co-host",
      body: "{hostName} đã gỡ phân công co-host của bạn {scope}",
    },  }
}
