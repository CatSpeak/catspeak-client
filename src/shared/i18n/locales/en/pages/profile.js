export default {
  sidebar: {
    personalInfo: "Personal Information",
    instructor: "Instructor",
    lecturer: "Lecturer",
    organization: "Organization",
    setting: "Setting",
    billing: "Payment History",
    menu: "Profile Menu",
  },
  personalInfo: {
    title: "Personal Information",
    avatar: "Avatar",
    username: "Username",
    email: "Email",
    accountType: "Account type",
    nickname: "Nickname",
    country: "Country",
    selectCountry: "Select country",
    searchCountry: "Search country...",
    searchPhoneCode: "Search phone code...",
    level: "Your Level",
    accountAndPrivacy: "Account and Privacy",
    password: "Password",
    address: "Address",
    save: "Save",
    enterUsername: "Enter username...",
    enterNickname: "Enter nickname...",
    enterAddress: "Enter your address...",
    enterEmail: "Enter email address...",
    enterPhoneNumber: "Enter phone number...",
    dateOfBirth: "Date of Birth",
    edit: "Edit",
    reset: "Reset",
    phoneNumber: "Phone number",
    otpSentToEmail: "We've sent a 6-digit confirmation code to your email",
    verifyAndSave: "VERIFY & SAVE",
    cancel: "Cancel",
    otpRequired: "OTP verification is required",
    otpLabel: "OTP Code",
    otpPlaceholder: "Enter 6-digit OTP code",
    currentPassword: "Current password",
    newPassword: "New password",
    confirmPassword: "Confirm new password",
    newPasswordMinLength: "New password must be at least 6 characters",
    newPasswordSameAsCurrent: "New password cannot be the same as current password",
    newPasswordLengthValid: "Password length is valid",
    passwordsMismatch: "Passwords do not match",
    passwordsMatch: "Passwords match",
    otpInvalid: "OTP is invalid or expired",
    currentIncorrect: "Current password is incorrect",
    allFieldsRequired: "All password fields are required",
    updatingAvatar: "Updating avatar...",
    updateAvatarSuccess: "Avatar updated successfully",
    updateAvatarError: "Failed to update avatar",
    updatingMeetingAvatar: "Updating meeting avatar...",
    updateMeetingAvatarSuccess: "Meeting avatar updated successfully",
    updateMeetingAvatarError: "Failed to update meeting avatar",
    meetingAvatarLabel: "Meeting room avatar",
    meetingAvatarHint: "This avatar will be shown to others when you join a meeting room.",
    mainAvatarLabel: "Main Account Avatar",
    changeMeetingAvatar: "Change Meeting Avatar",
    clickToChangeMeetingAvatar: "Click to change meeting avatar",
    updatingCover: "Updating cover photo...",
    updateCoverSuccess: "Cover photo updated successfully",
    updateCoverError: "Failed to update cover photo",
    editCover: "Edit cover photo",
    invalidImageFormat: "Please select a valid image file",
    coverSizeLimit: "Cover image size must not exceed 5MB",
  },
  instructor: {
    title: "Instructor",
    yourInfo: "Your Information",
    fullName: "Full name",
    email: "Email",
    address: "Your address",
    phoneNumber: "Your phone number",
    nationality: "Nationality",
    yourLanguage: "The language you teach",
    languageTeach: "Teaching Languages",
    chooseLanguage: "Choose the languages you will teach",
    selectLanguages: "Select languages",
    yourLevel: "Your level",
    selectLanguageFirst: "Please select a teaching language first",
    selectLevel: "Select level",
    nativeLanguage: "Native Language",
    selectNativeLanguage: "Select native language",
    idCard: "ID's card",
    idFront: "Front",
    idBack: "Back",
    idWarning:
      "After confirming and submitting your information, your identification details can no longer be changed!",
    background: "Your background and Credentials",
    introduceYourself: "Introduce yourself here",
    introPlaceholder:
      "To maintain a professional image and reflect the ideal qualities of a teacher. (≤ 200-300 chr)",
    uploadCredentials: "Upload your Credentials",
    credentialsNote:
      "You must provide proof or certificates that match the qualifications you selected above. The uploaded file should be in PDF format, clear, and detailed. STRICTLY PROHIBITED: Any form of fake certifications.",
    uploadVideo: "Upload your video introduce yourself",
    videoLabel: "Video",
    videoNote:
      "We recommend uploading a short 1-3 minute video introducing yourself to attract and impress potential students. The video should be in landscape (mp4) format! and under 500MB.",
    certify:
      "I certify that the information provided is accurate and agree to comply with the platform's regulations.",
    submit: "Submit",
    edit: "Edit",
    addCredential: "Add credential",
    selectNationality: "Select nationality",
    inputFieldPlaceholder: "Input field",
    notAppliedTitle: "You haven't applied yet",
    notAppliedDescription:
      "Share your expertise with students around the world. Apply to become an instructor and start teaching today.",
    applyNow: "Apply Now",
    submitting: "Submitting...",
    updating: "Updating...",
    statusPending: "Application Pending",
    statusPendingDesc:
      "Your application is being reviewed. We'll notify you once a decision is made.",
    statusApproved: "Application Approved",
    statusApprovedDesc:
      "Congratulations! Your instructor application has been approved.",
    switchToTeacherTitle: "Application Approved",
    switchToTeacherDesc:
      "Your instructor application has been approved. Switch to your teacher account to manage teaching content.",
    switchToTeacherAccount: "Switch to Teacher Account",
    switching: "Switching...",
    statusRejected: "Application Rejected",
    statusRejectedDesc:
      "Unfortunately, your application was not approved at this time.",
    statusRequestEdit: "Changes Requested",
    statusRequestEditDesc:
      "The reviewer has requested changes to your application. Please update the fields below and resubmit.",
    rejectReason: "Reason",
    bannedUntil: "Suspended until",
    editNote: "Note from reviewer",
    resubmit: "Resubmit application",
    noCredentials: "No credentials",
    document: "Document",
    attachedFile: "Attached File",
    pdfDocument: "PDF Document",
    remove: "Remove",
    requiredField: "This field is required",
    selectLanguagesError: "Please select teaching languages",
    selectLevelError: "Please select proficiency level for each language",
    selectAtLeastOneTag: "Please select at least 1 tag",
    tagsUpdatedSuccess: "Tags updated successfully",
    tagsUpdatedError: "Failed to update tags",
    certifyError: "Please check the box to agree with our terms.",
    formInvalid: "Please fill all required fields before submitting.",
    credentialSizeLimit: "Each credential must be smaller than {max}MB (current {actual}MB).",
    credentialTypeError: "Credentials must be in PDF format.",
    credentialLimit: "Maximum 4 credential files.",
    videoSizeLimit: "Video must be smaller than {max}MB (current {actual}MB).",
    editInfo: "Edit",
    cancelEditChanges: "Discard changes",
    cancelPendingRequest: "Cancel pending request",
    cancelPendingTitle: "Cancel pending request?",
    cancelPendingConfirm:
      "Cancel the pending draft? Your approved profile stays live and unchanged.",
    teachingUpdateOverwriteSuccess:
      "Pending request updated (previous request overwritten). Your approved profile keeps serving while awaiting review.",
    teachingSubmitSuccess:
      "Pending request updated (previous request overwritten). Your approved profile keeps serving while awaiting review.",
    teachingCancelSuccess: "Pending request cancelled.",
    approvedEditingHint:
      "Editing teaching content. Press Save to send for admin review (overwrites the previous request if any), or Discard changes to revert.",
    pendingBadge: "Awaiting admin review",
    pendingUpdateTitle: "Teaching content changes awaiting admin review",
    pendingUpdateDesc:
      "Your approved profile keeps showing and receiving students normally. Changes take effect after approval. You can still press Edit to overwrite the previous request.",
    previewCredential: "Click to preview certificate",
    openNewTab: "Open in new tab",
    download: "Download",
    previewLoading: "Loading preview...",
    approvedPersonalInfo: "Personal information",
    approvedVerification: "Information to verify",
    approvedBankAccount: "Bank account",
    approvedLanguageCol: "Language",
    approvedLevelCol: "Level",
    approvedCredentialCol: "Certificate",
    approvedStatusCol: "Status",
    approvedActionsCol: "Actions",
    approvedAddLanguage: "Add language",
    approvedUpdate: "Update",
    approvedChange: "Change",
    approvedView: "View",
    approvedStatusApproved: "Approved",
    approvedStatusPending: "Pending",
    approvedStatusRejected: "Rejected",
    approvedStatusCancelled: "Cancelled",
    approvedSentAt: "Submitted on",
    approvedNoVideo: "No intro video yet",
    approvedOptions: "Options",
    approvedReplaceVideo: "Replace video",
    approvedDeleteVideo: "Delete video",
    approvedUploadVideo: "Choose video",
    approvedDeleteVideoTitle: "Delete intro video",
    approvedDeleteVideoMessage:
      "Are you sure you want to delete your intro video? This action cannot be undone.",
    approvedDeleteVideoConfirm: "Delete",
    approvedVideoReplaced: "Intro video updated successfully",
    approvedVideoRemoved: "Intro video deleted",
    approvedVideoError: "Could not update the video. Please try again.",
    approvedVideoTypeError: "Only MP4 or MOV files are supported.",
    approvedVideoSizeError: "Video must be smaller than 50MB.",
    approvedVideoInvalidError: "Invalid video file.",
    competencyProfileTitle: "Teaching Competency Profile",
    competencyProfileSubtitle:
      "Manage specialties, teaching philosophy, education, experience, and public credentials.",
    editCompetencyProfile: "Edit Competency Profile",
    viewPublicProfile: "View Public Profile",
    headlineLabel: "Professional Headline",
    teachingMottoLabel: "Teaching Motto / Philosophy",
    credentialsSummary: "Professional Credentials Summary",
    certificatesLabel: "Certificates & Credentials",
    addCertificate: "Add Certificate",
    educationLabel: "Education",
    addEducation: "Add Education",
    experienceLabel: "Teaching Experience",
    addExperience: "Add Experience",
    teachingMethodsLabel: "Teaching Methods",
    competencySaveSuccess: "Competency profile updated successfully",
    competencySaveError: "Could not save competency profile. Please try again.",
    approvedSaveChanges: "Save changes",
    approvedCancelEdit: "Cancel",
    approvedFullNameRequired: "Please enter your full name",
    approvedFullNameLength: "Full name must be between 2 and 100 characters",
    approvedFullNameInvalid:
      "Full name can only contain letters, spaces, hyphens and apostrophes",
    approvedNationalityRequired: "Please select your nationality",
    approvedAddressRequired: "Please enter your address",
    approvedAddressLength: "Address must be between 5 and 255 characters",
    approvedIntroMax: "Introduction must be at most 300 characters",
    approvedPersonalInfoSaved: "Personal information updated successfully",
    approvedPersonalInfoSaveError: "Could not update personal information",
    contactChangeEmailTitle: "Change Email",
    contactVerifyEmailTitle: "Verify new Email",
    contactChangePhoneTitle: "Change Phone Number",
    contactVerifyPhoneTitle: "Verify new Phone Number",
    contactCurrentEmail: "Current email",
    contactNewEmail: "New email",
    contactCurrentPhone: "Current phone number",
    contactNewPhone: "New phone number",
    contactEmailHelper:
      "The new email will need to be verified before it takes effect.",
    contactPhoneHelper:
      "The new phone number will need to be verified before it takes effect.",
    contactOtpSentTo: "The verification code has been sent to {value}.",
    contactOtpLabel: "Enter verification code",
    contactResendCountdown: "Resend code in {time}",
    contactResend: "Resend code",
    contactCancel: "Cancel",
    contactSendOtp: "Send verification code",
    contactConfirm: "Confirm",
    contactEmailInvalid: "Email is not in a valid format",
    contactEmailSame: "The new email must differ from the current email",
    contactEmailExists:
      "This email is already used by another account",
    contactPhoneInvalid: "Phone number is invalid",
    contactPhoneSame:
      "The new phone number must differ from the current one",
    contactPhoneExists:
      "This phone number is already used by another account",
    contactPhoneRateLimit:
      "You can only change your phone number once per month.",
    contactOtpRequired: "Please enter the verification code",
    contactOtpInvalid: "The verification code is invalid or has expired",
    contactOtpAttemptsExceeded:
      "You have entered the code incorrectly too many times. Please request a new code.",
    contactRequestError: "Could not send the verification code. Please try again.",
    contactConfirmError: "Could not update the information. Please try again.",
    contactEmailUpdated: "Email updated successfully",
    contactPhoneUpdated: "Phone number updated successfully",
    bankChangeTitle: "Change bank account",
    bankVerifyTitle: "Verify new bank account",
    bankCurrentAccount: "Current account",
    bankLabel: "Bank",
    bankNewAccountNumber: "New account number",
    bankAccountNumber: "Account number",
    bankAccountHolder: "Account holder name",
    bankAccountNumberPlaceholder: "Enter account number...",
    bankSelectPlaceholder: "Select a bank",
    bankSearchPlaceholder: "Search banks...",
    bankCheckAccount: "Check account",
    bankStep1Banner:
      "The system will verify the account details before sending you a verification code.",
    bankStep2Banner:
      "The new account details have been verified with the bank. Please review the information below carefully before sending the verification code.",
    bankVerifiedInfo: "New account details (verified)",
    bankCurrentBadge: "Current",
    bankConsent:
      "I confirm that the account details above are correct and agree to use this account to receive payments from Cat Speak.",
    bankOtpWillSendTo:
      "The verification code (OTP) will be sent to your verified phone number:",
    bankOtpHelper:
      "The verification code has been sent to your current verified phone number by default.",
    bankOtpInputLabel: "Enter verification code (OTP)",
    bankAccountNumberInvalid: "The account number must be 6 to 20 digits",
    bankNoAccount: "No bank account yet",
    bankOtpRequired: "Please enter the verification code",
    bankOtpInvalid: "The verification code is invalid or has expired",
    bankOtpAttemptsExceeded:
      "You have entered the code incorrectly too many times. Please request a new code.",
    bankVerifyError:
      "We could not verify the bank account. Please check the account number and try again.",
    bankRequestError: "Could not send the verification code. Please try again.",
    bankConfirmError: "Could not update the bank account. Please try again.",
    bankUpdated: "Bank account updated successfully",
    idCardDrawerTitle: "Update ID card",
    idCardCurrentInfo: "Current information",
    idCardWantedInfo: "Information to update",
    idCardFrontNew: "New front side",
    idCardBackNew: "New back side",
    idCardDropHint: "Drag and drop or",
    idCardChooseFile: "Choose file",
    idCardConstraint: "JPG, PNG up to 5MB",
    idCardSubmit: "Submit for verification",
    idCardUpdatedNote: "Image updated",
    idCardUpdatedToast: "ID card images updated successfully",
    idCardUpdateError:
      "Could not update the ID card images. Please try again.",
    idCardFileType: "Only JPG, PNG, WEBP images are accepted",
    idCardFileSize: "The image must not exceed 5MB",
    idCardFileInvalid: "The image file is invalid or unreadable",
    languageSelectPlaceholder: "Select a language",
    languageLevelPlaceholder: "Select a level",
    languageCertificateLabel: "Certificate / Degree",
    languageCertificateNewLabel: "New certificate / degree",
    languageCertificateConstraint: "PDF up to 5MB",
    languageUpdateTitle: "Update {language}",
    languageNewLevelLabel: "New level",
    languageInfoBox:
      "Your previously approved information remains in effect until the new update request is approved.",
    languageRequestSubmit: "Submit request",
    languageRequestSubmitted:
      "Request sent for administrator review. Please wait a moment.",
    languageRequestError: "Could not submit the request. Please try again.",
    languageRequestConflict:
      "This language already has a pending request. Please delete the old request before submitting a new one.",
    languageCertificateFileType: "Only PDF files are accepted",
    languageCertificateFileSize: "The certificate must not exceed 5MB",
    languageCertificateFileInvalid: "The PDF file is invalid or unreadable",
    approvedNativeLanguageRequired: "Please select your native language",
    approvedRequestDetailTitle: "Update request details",
    approvedRequestCurrentInfo: "Current information",
    approvedRequestWantedInfo: "Request information",
    approvedRequestNewLevel: "New level",
    approvedRequestNewCredential: "New certificate",
    approvedRequestPendingInfo:
      "Your request is awaiting administrator review. Your currently approved information remains in effect.",
    approvedRequestRejectReason: "Rejection reason",
    approvedRequestDelete: "Delete request",
    approvedRequestClose: "Close",
    approvedRequestDeleteTitle: "Delete request",
    approvedRequestDeleteMessage:
      "Are you sure you want to delete this request? This action cannot be undone.",
    approvedRequestDeleteConfirm: "Delete",
    approvedRequestDeleted: "Request deleted",
    approvedRequestDeleteError: "Could not delete the request. Please try again.",
  },
  friends: {
    teacher: "Teacher",
    member: "Member",
    searchPlaceholder: "Search friends...",
    options: "Options",
    filter: "Filters",
    filterTitle: "Filter friends",
    accountType: "Role",
    allRoles: "All",
    applyFilter: "Apply",
    clearFilter: "Clear",
    pendingIncoming: "Awaiting your approval",
    pendingOutgoing: "Sent by you",
    subTabs: {
      all: "All friends",
      following: "Following",
      followers: "Followers",
      pending: "Friend requests",
      find: "Find friends",
      teachers: "Teachers",
      members: "Members",
    },
    actions: {
      addFriend: "Add friend",
      requestSent: "Request sent",
      cancelRequest: "Withdraw",
      cancelTitle: "Withdraw request",
      cancelMessage:
        "Withdraw the friend request sent to {name}? You will also unfollow this person.",
      keepRequest: "Keep",
      cancelSuccess: "Request withdrawn",
      accept: "Accept",
      decline: "Decline",
      message: "Message",
      unfollow: "Unfollow",
      followBack: "Follow back",
      unfriend: "Unfriend",
      chatError: "Unable to start conversation",
      error: "An error occurred",
    },
    empty: {
      noData: "No data available",
      noFriends: "No friends yet.",
      noFollowing: "Not following anyone yet.",
      noFollowers: "No followers yet.",
      noPending: "No pending friend requests.",
      noSent: "No sent requests yet.",
      noRecommendations: "No recommendations available.",
      noSearchResults: "No results found for \"{query}\".",
      noTeachers: "No teachers yet.",
      noMembers: "No members yet.",
    },
  },
  notifications: {
    friend_request: {
      title: "New Friend Request",
      body: "{requesterName} sent you a friend request.",
    }
  }
}
