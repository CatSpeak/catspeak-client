      ...
      dispatch(
        reelsApi.util.updateQueryData(endpointName, originalArgs, (draft) => {
          ...
          const index = reelsList.findIndex(
            (reel) => String(reel.reelId) === String(reelId),
          );
  ...
  endpoints: (builder) => ({
    ...
    getReelsFeed: builder.query({
      query: ({
        ...
        excludeReelIds.forEach((id) => {
          params.append("excludeReelIds", String(id));
        });
      ...
      merge: (currentCache, incomingResponse, { arg }) => {
        ...
        const existingIds = new Set(
          currentReels.map((reel) => String(reel.reelId || reel.id)),
        );
      ...
      forceRefetch({ currentArg, previousArg }) {
        const currentExcludeIds = currentArg?.excludeReelIds || [];
        const previousExcludeIds = previousArg?.excludeReelIds || [];

        return (
          currentArg?.sourceFilter !== previousArg?.sourceFilter ||
          currentArg?.search !== previousArg?.search ||
          currentArg?.pageSize !== previousArg?.pageSize ||
          currentExcludeIds.length !== previousExcludeIds.length ||
          currentExcludeIds.some(
            (id, index) => String(id) !== String(previousExcludeIds[index]),
          )
        );
      },
    ...
    getReelsByChallenge: builder.query({
      ...
      merge: (currentCache, incomingResponse, { arg }) => {
        ...
        const currentReels = getReelList(currentCache);
        const existingIds = new Set(
          currentReels.map((reel) => String(reel.reelId)),
        );
        incomingReels.forEach((reel) => {
    ...
    deleteReel: builder.mutation({
      ...
      async onQueryStarted(reelId, { dispatch, getState, queryFulfilled }) {
        const patchResults = removeCachedReelFromLists(
          dispatch,
          getState,
          reelId,
        );
    ...
    getBookmarkedReels: builder.query({
      query: (playlistId) =>
        playlistId
          ? `/Reels/bookmarked?playlistId=${playlistId}`
          : `/Reels/bookmarked`,
      providesTags: (result, error, playlistId) => [
        { type: "Playlists", id: playlistId || "ALL" },
      ],
    }),
        ...
        try {
          ...
          if (defaultResult.data) {
            defaultResult.data.forEach((item) => {
              bookmarksMap.set(String(item.reelId), {
                ...item,
                __playlistIds: [null],
              });
            });
          ...
          const playlistPromises = playlists.map((p) =>
            fetchWithBQ(`/Reels/bookmarked?playlistId=${p.playlistId}`),
          );
            ...
            if (result.data) {
              ...
              result.data.forEach((item) => {
                ...
                if (bookmarksMap.has(key)) {
                  ...
                  bookmarksMap.set(key, {
                    ...item,
                    __playlistIds: [playlistId],
                  });
                }
              });
            }
    ...
    notInterestedReel: builder.mutation({
      ...
      // Optimistic update
      async onQueryStarted({ reelId }, { dispatch, getState, queryFulfilled }) {
        const patchResults = patchCachedReelLists(
          dispatch,
          getState,
          reelId,
          (reel) => {
